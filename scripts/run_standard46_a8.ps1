...
try {
    ...
    $logRoot = Join-Path "files/standard46_a8_logs" $timestamp
    New-Item -ItemType Directory -Force -Path $logRoot | Out-Null

    $algorithmName = "ELP_RL_Standard_standard46_a8"
    if ($RandomPolicy) {
        $algorithmName = "ELP_RL_Standard_standard46_a8_random"
    }

    $env:ELP_STANDARD46_PROFILE = "a8"
    $env:ELP_IS_EXP = "1"
    ...
    $env:ELP_EXP_REMARK = "Standard4.6 A8 core | random_policy=" + [string][bool]$RandomPolicy + " | seeds=" + $FixedSeeds + " | fixed_steps=" + [string]($G * $TMax)
    $env:ELP_PRINT_TELEMETRY = "1"

    # A8 固定语义：只关闭 Two-stage heavy actions，其余外层机制保持开启。
    $env:ELP_TWO_STAGE_HEAVY_ACTIONS_ENABLED = "0"
    $env:ELP_REHEAT_ENABLED = "1"
    $env:ELP_SEGMENT_INSERT_LIGHT_ENABLED = "1"
    $env:ELP_FINAL_ELITE_PUSH_ENABLED = "1"
    $env:ELP_MID_STRUCTURAL_SHOT_ENABLED = "1"
    ...
    $header = "[STANDARD46_A8] start algorithm=" + $algorithmName + " | G=" + $G + " | TMax=" + $TMax + " | seeds=" + $FixedSeeds
    Write-Host $header
    Add-Content -Path $logPath -Value $header -Encoding UTF8

    if ($DryRun) {
        $dryLine = "[STANDARD46_A8] DryRun enabled, skip execution"
        Write-Host $dryLine
    ...
    $tail = "[STANDARD46_A8] end algorithm=" + $algorithmName + " | exit_code=" + $exitCode
    Write-Host $tail
    Add-Content -Path $logPath -Value $tail -Encoding UTF8

    if ($exitCode -ne 0) {
        throw ("Standard4.6 A8 run failed with exit code " + $exitCode)
    }
}
finally {
    Pop-Location
