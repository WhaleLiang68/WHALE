...
class ELP:
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0):
        ...
        if self.tune_profile not in {"B0", "E1", "D1", "ED1"}:
            logger.warning(f"Unknown ELP_TUNE_PROFILE={self.tune_profile}, fallback to ED1")
            self.tune_profile = "ED1"
        self.profile_elp = self.tune_profile in {"E1", "ED1"}
        self.profile_dqn = self.tune_profile in {"D1", "ED1"}
...
if __name__ == "__main__":
    ...
    exp_remark = "ELP+DQN,移除了 facility_insert (5) 和 facility_shuffle (7),分箱明确"
    exp_number = 50
    is_exp = True
