...
STANDARD46_RUN_DEFAULTS = {
    "ELP_IS_EXP": "1",
    "ELP_EXP_INSTANCE": "Du62",
    "ELP_G": "600",
    "ELP_T_MAX": "240",
    "ELP_T_INITIAL": "3000.0",
    "ELP_K_HIST": "5.0",
    "ELP_BIN_WIDTH": "50.0",
    "ELP_ADAPTIVE_BIN_WIDTH_ENABLED": "0",
    "ELP_EXP_NUMBER": "5",
    "ELP_RL_AGENT": "dqn",
    "ELP_PRINT_TELEMETRY": "1",
    "ELP_SAVE_TELEMETRY_CSV": "1",
    "ELP_SAVE_EXPERIMENT_RESULT": "1",
}

STANDARD46_PROFILES = {
    "a8": {
        "algorithm": "ELP_RL_Standard_standard46_actions8",
        "runtime_config_name": "standard46_actions8",
        "remark": (
            "Standard4.6 A8 core with rewritten 8-action DQN neighborhood; "
            "no main two-stage heavy actions"
        ),
        "env": {
            "ELP_TWO_STAGE_HEAVY_ACTIONS_ENABLED": "0",
            "ELP_REHEAT_ENABLED": "1",
            "ELP_FINAL_ELITE_PUSH_ENABLED": "1",
            "ELP_MID_STRUCTURAL_SHOT_ENABLED": "1",
        },
        "runtime_defaults": {
            "reheat_enabled": True,
            "elite_actions": [3, 4, 5, 0],
            "elite_action_trials": {3: 2, 4: 2, 5: 1, 0: 1},
            "mid_structural_shot_action_ids": [4, 5, 3],
            "local_search_trigger_action_ids": [3, 4, 5],
            "topk_guided_trigger_action_ids": [3, 4, 5, 0],
            "two_stage_heavy_action_ids": [],
            "two_stage_local_proxy_action_ids": [],
            "action_budget_controller_enabled": True,
            "reward_eval_cost_penalty": 0.02,
        },
    },
    "light": {
        "algorithm": "ELP_RL_Standard_standard46_light",
        "runtime_config_name": "standard46_light",
        "remark": (
            "Standard4.6 light core: no two-stage heavy actions, no reheat, "
            "no final_elite_push, no mid_structural_shot"
        ),
        "env": {
            "ELP_TWO_STAGE_HEAVY_ACTIONS_ENABLED": "0",
            "ELP_REHEAT_ENABLED": "0",
            "ELP_FINAL_ELITE_PUSH_ENABLED": "0",
            "ELP_MID_STRUCTURAL_SHOT_ENABLED": "0",
        },
        "runtime_defaults": {
...
STANDARD46_CONTROLLED_SWITCHES = {
...
STANDARD46_STALE_EXP_ALGORITHMS = {
    "standard46_a8_lightshot_midshot_dqn",
}

STANDARD46_STALE_REMARK_FRAGMENTS = (
    "light mid_structural_shot",
    "light-shot",
    "lightshot",
)


def _standard46_has_non_empty_env(key):
    return bool(os.getenv(key, "").strip())


def _standard46_should_replace_exp_algorithm(value):
    normalized = str(value or "").strip().lower()
    if not normalized:
        return True
    return normalized in STANDARD46_STALE_EXP_ALGORITHMS or "lightshot" in normalized


def _standard46_should_replace_exp_remark(value):
    normalized = str(value or "").strip().lower()
    if not normalized:
        return True
    return any(fragment in normalized for fragment in STANDARD46_STALE_REMARK_FRAGMENTS)


def _standard46_random_base_seed():
    # 默认随机生成 base seed，避免直接运行时总是复用同一组实验。
    return str(10_000_000 + secrets.randbelow(90_000_000))


def _standard46_apply_seed_defaults():
    fixed_seeds = os.getenv("ELP_FIXED_SEEDS")
    if fixed_seeds is not None and not fixed_seeds.strip():
        os.environ.pop("ELP_FIXED_SEEDS", None)
    if _standard46_has_non_empty_env("ELP_FIXED_SEEDS"):
        os.environ["ELP_EXP_NUMBER"] = str(
            len([seed for seed in os.environ["ELP_FIXED_SEEDS"].split(",") if seed.strip()])
        )
        return
    if not _standard46_has_non_empty_env("ELP_BASE_SEED"):
        os.environ["ELP_BASE_SEED"] = _standard46_random_base_seed()


def _standard46_seed_summary():
    if _standard46_has_non_empty_env("ELP_FIXED_SEEDS"):
        return f"fixed_seeds={os.environ['ELP_FIXED_SEEDS']}"
    return f"base_seed={os.environ['ELP_BASE_SEED']}, exp_number={os.environ['ELP_EXP_NUMBER']}"


def _standard46_fixed_step_count():
    return int(os.environ["ELP_G"]) * int(os.environ["ELP_T_MAX"])


def _standard46_is_random_policy():
    if os.getenv("ELP_RL_AGENT", "dqn").strip().lower() != "qlearning":
        return False
    return (
        os.getenv("ELP_DQN_EPSILON_START", "").strip() == "1"
        and os.getenv("ELP_DQN_EPSILON_MIN", "").strip() == "1"
        and os.getenv("ELP_DQN_EPSILON_DECAY", "").strip() == "1"
    )


def _standard46_profile_name():
    profile_name = os.getenv("ELP_STANDARD46_PROFILE", "a8").strip().lower()
    if profile_name not in STANDARD46_PROFILES:
        valid_names = ", ".join(sorted(STANDARD46_PROFILES))
        raise ValueError(
            f"Invalid ELP_STANDARD46_PROFILE={profile_name!r}; expected one of: {valid_names}"
        )
    return profile_name


def _standard46_apply_direct_run_defaults():
    for key, value in STANDARD46_RUN_DEFAULTS.items():
        os.environ.setdefault(key, value)
    _standard46_apply_seed_defaults()

    if _env_flag("ELP_STANDARD46_RANDOM_POLICY", False):
        os.environ["ELP_RL_AGENT"] = "qlearning"
        os.environ["ELP_DQN_EPSILON_START"] = "1"
        os.environ["ELP_DQN_EPSILON_MIN"] = "1"
        os.environ["ELP_DQN_EPSILON_DECAY"] = "1"
    elif os.environ["ELP_RL_AGENT"].strip().lower() == "dqn":
        # DQN 运行默认清理随机策略残留的 epsilon=1，避免旧 shell 环境污染实验。
        os.environ.pop("ELP_DQN_EPSILON_START", None)
        os.environ.pop("ELP_DQN_EPSILON_MIN", None)
        os.environ.pop("ELP_DQN_EPSILON_DECAY", None)


def _apply_standard46_profile():
    _standard46_apply_direct_run_defaults()
    profile = STANDARD46_PROFILES[_standard46_profile_name()]

    # 这些开关定义 4.6 的实验母本，主动覆盖旧 shell 环境，避免伪配置污染结果。
    for key in STANDARD46_CONTROLLED_SWITCHES:
        os.environ[key] = profile["env"][key]

    random_policy = _standard46_is_random_policy()
    configured_algorithm = os.getenv("ELP_EXP_ALGORITHM", "").strip()
    replace_algorithm = _standard46_should_replace_exp_algorithm(configured_algorithm)
    if random_policy and configured_algorithm and (not replace_algorithm) and "random" not in configured_algorithm.lower():
        raise ValueError(
            "Standard4.6 检测到随机策略已激活，但 ELP_EXP_ALGORITHM 未包含 random。"
            "如果这是有意的随机对照，请把算法名写入 random；否则请清理 "
            "ELP_STANDARD46_RANDOM_POLICY/ELP_RL_AGENT/ELP_DQN_EPSILON_* 后重新运行。"
        )

    algorithm = profile["algorithm"]
    if random_policy:
        algorithm = f"{algorithm}_random"
    if replace_algorithm:
        os.environ["ELP_EXP_ALGORITHM"] = algorithm

    remark = (
        f"{profile['remark']} | random_policy={random_policy} | "
        f"T={os.environ['ELP_T_INITIAL']} | k_hist={os.environ['ELP_K_HIST']} | "
        f"bin_width={os.environ['ELP_BIN_WIDTH']} | "
        f"adaptive_bin_width={os.environ['ELP_ADAPTIVE_BIN_WIDTH_ENABLED']} | "
        f"{_standard46_seed_summary()} | fixed_steps={_standard46_fixed_step_count()}"
    )
    if _standard46_should_replace_exp_remark(os.getenv("ELP_EXP_REMARK", "")):
        os.environ["ELP_EXP_REMARK"] = remark

    runtime_config.ELP_RUNTIME_CONFIG_NAME = profile["runtime_config_name"]
    defaults = getattr(runtime_config, "ELP_RUNTIME_DEFAULTS", None)
    if isinstance(defaults, dict):
        defaults.update(profile["runtime_defaults"])
...
class ELP:
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0):
        ...
        self.temperature_floor_samples = 20
        ...
        self.action_recipes = {action_idx: [] for action_idx in range(8)}
        self.feasible_action_ids = [0, 1, 2, 3, 4, 5]
        self.infeasible_action_ids = [100, 101, 102]
        self.all_action_ids = list(self.feasible_action_ids) + list(self.infeasible_action_ids)
        self.rl_action_dim = max(len(self.feasible_action_ids), len(self.infeasible_action_ids))
        self.temperature_floor_action_ids = [4, 3, 1, 0, 5]
        self.valid_actions = list(self.feasible_action_ids)
        self.phase_action_ids = {
            "early": [4, 5, 2, 3, 1, 0],
            "mid": [3, 4, 5, 1, 0, 2],
            "late": [3, 1, 0, 4, 5, 2],
        }
        self.infeasible_phase_action_ids = {
            "repair": [100, 101, 102],
        }
        self.logger = logger
        ...
        self.mid_structural_shot_action_ids = tuple(
            int(action_idx)
            for action_idx in _runtime_default("mid_structural_shot_action_ids", [4, 5, 3])
        )
        ...
        self.new_action_candidate_cap = max(
            4,
            _env_int("ELP_NEW_ACTION_CANDIDATE_CAP", 6),
        )
        self.action_budget_controller_enabled = _env_flag(
            "ELP_ACTION_BUDGET_CONTROLLER_ENABLED",
            bool(_runtime_default("action_budget_controller_enabled", True)),
        )
        self.action_budget_light_cap = max(1, _env_int("ELP_ACTION_BUDGET_LIGHT_CAP", 1))
        self.action_budget_medium_cap = max(
            self.action_budget_light_cap,
            _env_int("ELP_ACTION_BUDGET_MEDIUM_CAP", 3),
        )
        self.action_budget_heavy_cap = max(
            self.action_budget_medium_cap,
            _env_int("ELP_ACTION_BUDGET_HEAVY_CAP", min(6, self.new_action_candidate_cap)),
        )
        self.action_budget_medium_no_improve = max(
            5,
            _env_int("ELP_ACTION_BUDGET_MEDIUM_NO_IMPROVE", max(20, self.t_max // 4)),
        )
        self.action_budget_heavy_no_improve = max(
            self.action_budget_medium_no_improve + 1,
            _env_int("ELP_ACTION_BUDGET_HEAVY_NO_IMPROVE", max(60, self.t_max // 2)),
        )
        self.action_budget_min_selected = max(
            1,
            _env_int("ELP_ACTION_BUDGET_MIN_SELECTED", 20),
        )
        self.action_budget_medium_min_final_rate = min(
            1.0,
            max(0.0, _env_float("ELP_ACTION_BUDGET_MEDIUM_MIN_FINAL_RATE", 0.015)),
        )
        self.action_budget_heavy_min_final_rate = min(
            1.0,
            max(0.0, _env_float("ELP_ACTION_BUDGET_HEAVY_MIN_FINAL_RATE", 0.030)),
        )
        self.reward_eval_cost_penalty = max(
            0.0,
            _env_float(
                "ELP_REWARD_EVAL_COST_PENALTY",
        ...
        self._last_action_eval_count = 0
        self._last_action_budget_cap = self.action_budget_light_cap
        self._last_action_budget_level = "light"
        self._last_reward_eval_cost_penalty = 0.0
        ...
        self.action_labels = {
            0: "facility_pair_swap",
            1: "intra_bay_facility_insert",
            2: "bay_reorder",
            3: "intra_bay_segment_insert",
            4: "cross_bay_relocate",
            5: "adjacent_boundary_block_repartition",
            100: "targeted_geometry_repair",
            101: "bay_count_adjust_by_flow",
            102: "boundary_repartition_for_repair",
        }
        self.action_mode_by_id = {
            action_idx: "feasible" for action_idx in self.feasible_action_ids
        }
        self.action_mode_by_id.update(
            {action_idx: "infeasible" for action_idx in self.infeasible_action_ids}
        )
        ...
        self.elite_actions = [3, 4, 5, 0]
        self.elite_action_trials = {3: 2, 4: 2, 5: 1, 0: 1}
        ...
        self.diversify_recipe = [5, 4, 101, 100]
        self.bin_width = max(1e-9, _env_float("ELP_BIN_WIDTH", 50.0))
        self.adaptive_bin_width_enabled = _env_flag("ELP_ADAPTIVE_BIN_WIDTH_ENABLED", False)
        ...
        self.action_telemetry = {
            ...
            for action_idx in sorted(set(self.all_action_ids) | set(self.elite_actions))
        }
        ...
        self.elite_actions = [
            int(action_idx)
            for action_idx in _runtime_default("elite_actions", [3, 4, 5, 0])
        ]
        ...
        self.two_stage_heavy_action_ids = set(
            _env_int_list("ELP_TWO_STAGE_HEAVY_ACTION_IDS") or []
        )
        if self.two_stage_heavy_actions_enabled:
            raise RuntimeError(
                "Standard4.6 rewritten action set does not support legacy two-stage heavy actions."
            )
        ...
        self.local_search_trigger_action_ids = set(
            _env_int_list("ELP_LOCAL_SEARCH_TRIGGER_ACTION_IDS") or [3, 4, 5]
        )
        ...
        self.topk_guided_trigger_action_ids = set(
            _env_int_list("ELP_TOPK_TRIGGER_ACTION_IDS") or [3, 4, 5, 0]
        )
        ...
        logger.info(
            ...
            f"T_initial: {self.T_initial:.6f} | k_hist: {self.k_hist:.6f} | "
    ...
    def _current_action_mode(self, solution):
        if int(getattr(solution, "current_d_inf", 0)) > 0:
            return "infeasible"
        return "feasible"

    def _current_progress_phase_name(self):
        progress = min(max(float(getattr(self, "current_progress_ratio", 0.0)), 0.0), 1.0)
        if progress < 0.25:
            return "early"
        if progress < 0.75:
            return "mid"
    ...
    def _action_ids_for_mode(self, action_mode):
        if str(action_mode) == "infeasible":
            return list(self.infeasible_action_ids)
        return list(self.feasible_action_ids)

    def _current_action_ids(self, solution):
        return self._action_ids_for_mode(self._current_action_mode(solution))

    def _current_action_space_size(self, solution):
        return len(self._current_action_ids(solution))

    def _resolve_action_id(self, solution, local_action_idx):
        action_ids = self._current_action_ids(solution)
        local_action_idx = int(local_action_idx)
        if local_action_idx < 0 or local_action_idx >= len(action_ids):
            raise IndexError(f"Unsupported local action index: {local_action_idx}")
        return int(action_ids[local_action_idx])

    def _record_action_mode_visit(self, solution):
        ...
        previous_mode = getattr(self, "_last_recorded_action_mode", None)
        if action_mode == "infeasible":
            self.infeasible_step_count = int(getattr(self, "infeasible_step_count", 0)) + 1
            if previous_mode != "infeasible":
                self.infeasible_entry_count = int(getattr(self, "infeasible_entry_count", 0)) + 1
    ...
    def _new_action_telemetry_entry(self, action_idx):
        return {
            ...
            "action_mode": self.action_mode_by_id.get(action_idx, "unknown"),
            "selected": 0,
            "explore_selected": 0,
            "exploit_selected": 0,
            "accepted": 0,
            ...
            "budget_medium": 0,
            "budget_heavy": 0,
            "reward_eval_cost_penalty_sum": 0.0,
            "elite_selected": 0,
            ...
            "elite_eval_count_sum": 0.0,
            "elite_budget_cap_sum": 0.0,
            "elite_budget_light": 0,
            "elite_budget_medium": 0,
            "elite_budget_heavy": 0,
            "elite_reward_eval_cost_penalty_sum": 0.0,
        }
    ...
    def _record_action_selection(
        ...
        eval_count = max(0, int(getattr(self, "_last_action_eval_count", 0)))
        budget_cap = max(0, int(getattr(self, "_last_action_budget_cap", 0)))
        budget_level = str(getattr(self, "_last_action_budget_level", "light")).strip().lower()
        if budget_level not in {"light", "medium", "heavy"}:
            budget_level = "light"
        stats[f"{prefix}eval_count_sum"] += float(eval_count)
        stats[f"{prefix}budget_cap_sum"] += float(budget_cap)
        stats[f"{prefix}budget_{budget_level}"] += 1
        delta = 0.0
    ...
    def _record_action_final_outcome(
        ...
        eval_penalty = float(getattr(self, "_last_reward_eval_cost_penalty", 0.0))
        if np.isfinite(eval_penalty):
            stats[f"{prefix}reward_eval_cost_penalty_sum"] += eval_penalty
    ...
    def get_action_telemetry(self):
        ...
        for action_idx, stats in self.action_telemetry.items():
            ...
            for prefix in ("", "elite_"):
                ...
                item[f"{prefix}avg_eval_count"] = 0.0 if selected == 0 else stats[f"{prefix}eval_count_sum"] / selected
                item[f"{prefix}avg_budget_cap"] = 0.0 if selected == 0 else stats[f"{prefix}budget_cap_sum"] / selected
                item[f"{prefix}budget_light_rate"] = 0.0 if selected == 0 else stats[f"{prefix}budget_light"] / selected
                item[f"{prefix}budget_medium_rate"] = 0.0 if selected == 0 else stats[f"{prefix}budget_medium"] / selected
                item[f"{prefix}budget_heavy_rate"] = 0.0 if selected == 0 else stats[f"{prefix}budget_heavy"] / selected
                item[f"{prefix}avg_reward_eval_cost_penalty"] = (
                    0.0 if selected == 0 else stats[f"{prefix}reward_eval_cost_penalty_sum"] / selected
                )
    ...
    def format_action_telemetry(self):
        ...
        for action_idx, stats in ordered:
            lines.append(
                (
                    "Action {idx} [{name}]<{action_mode}> | main sel={selected} exr={explore_rate:.1%}/{exploit_rate:.1%} "
                    ...
                    "eval={avg_eval_count:.2f}/{avg_budget_cap:.2f} budget=L{budget_light_rate:.0%}/M{budget_medium_rate:.0%}/H{budget_heavy_rate:.0%} "
                    "avg_cand={avg_cand_delta:.2f} avg_final={avg_final_delta:.2f} avg_ls={avg_local_search_delta:.2f} "
                    "avg_r={avg_immediate_reward:.3f}->{avg_final_reward:.3f} ({avg_reward_uplift:+.3f}) eval_pen={avg_eval_penalty:.3f} "
                    ...
                    "eval={elite_avg_eval_count:.2f}/{elite_avg_budget_cap:.2f} "
                    ...
                    idx=action_idx,
                    name=stats["name"],
                    action_mode=stats.get("action_mode", "unknown"),
                    ...
                    avg_eval_count=stats["avg_eval_count"],
                    avg_budget_cap=stats["avg_budget_cap"],
                    budget_light_rate=stats["budget_light_rate"],
                    budget_medium_rate=stats["budget_medium_rate"],
                    budget_heavy_rate=stats["budget_heavy_rate"],
                    ...
                    avg_eval_penalty=stats["avg_reward_eval_cost_penalty"],
                    ...
                    elite_avg_eval_count=stats["elite_avg_eval_count"],
                    elite_avg_budget_cap=stats["elite_avg_budget_cap"],
    ...
    def _make_archive_entry(self, solution, lane):
        ...
        true_fitness = float(getattr(solution, "true_fitness", fitness))
    ...
    def _effective_elite_controls(self):
        ...
        for action_idx in list(trial_map.keys()):
            ...
            if phase == "late" and action_idx in {3, 4, 5}:
                scaled_trials += 1
            if phase == "final":
        ...
        for seed_solution in self._select_mid_structural_shot_seed_solutions():
            ...
            for action_idx in self.mid_structural_shot_action_ids:
                for _ in range(int(self.mid_structural_shot_trials_per_action)):
                    candidate = self._generate_standard46_new_action_candidate(
                        seed_solution,
                        action_idx,
                        phase="mid_shot",
                    )
    ...
    def _get_adaptive_bin_width(self, best_energy):
        if not self.adaptive_bin_width_enabled:
    ...
    def _sample_temperature_floor(self, n_samples=None, target_accept=None):
        ...
        allowed_actions = [
            local_idx
            for local_idx, action_idx in enumerate(self.feasible_action_ids)
        ...
        for _ in range(max(1, n_samples)):
            ...
            action_idx = self._resolve_action_id(base_solution, action_table_idx)
            candidate = self._generate_standard46_new_action_candidate(
                base_solution,
                action_idx,
                phase="temperature_floor",
            )
    ...
    def _generate_candidate_by_action_fallback(self, solution, action_idx, phase="main"):
        return self._generate_standard46_new_action_candidate(solution, action_idx, phase=phase)
    ...
    def _build_extra_state_context(self, solution, tm, permutation, bay_ranges, aspect_violation, infeasible_mask):
        n = max(1, len(permutation))
        areas = self._fit_state_feature_length(getattr(solution, "areas", []), n, fill_value=1.0)
        total_area = max(float(np.sum(areas)), 1e-8)
        bay_area_ratios = []
        bay_violation_means = []
        facility_to_bay = {}
        for bay_idx, (start, end) in enumerate(bay_ranges):
            ...
            for pos in range(start, end + 1):
                if pos < 0 or pos >= len(permutation):
                    continue
                facility_id = int(permutation[pos])
                facility_idx = facility_id - 1
                facility_to_bay[facility_id] = int(bay_idx)
                if 0 <= facility_idx < areas.size:
                    bay_area += float(areas[facility_idx])
                if 0 <= facility_idx < len(aspect_violation):
                    bay_violations.append(float(aspect_violation[facility_idx]))
                bay_facilities.append(facility_id)
            if bay_facilities:
                bay_area_ratios.append(float(bay_area) / total_area)
                bay_violation_means.append(float(np.mean(bay_violations)) if bay_violations else 0.0)

        bay_count = max(1, len(bay_area_ratios))
        target_area_ratio = 1.0 / float(bay_count)
        if bay_area_ratios:
            bay_area_arr = np.asarray(bay_area_ratios, dtype=np.float32)
            bay_area_std = float(np.std(bay_area_arr))
            bay_area_max_dev = float(np.max(np.abs(bay_area_arr - target_area_ratio)))
            max_bay_violation = float(np.max(np.asarray(bay_violation_means, dtype=np.float32)))
            mean_bay_violation = float(np.mean(np.asarray(bay_violation_means, dtype=np.float32)))
        else:
            bay_area_std = 0.0
            bay_area_max_dev = 0.0
            max_bay_violation = 0.0
            mean_bay_violation = 0.0

        top_flow_cross_ratio = 0.0
        top_flow_adjacent_ratio = 0.0
        if tm.ndim == 2 and tm.shape[0] == tm.shape[1] and tm.shape[0] >= n:
            pair_rows = []
            for i in range(n):
                for j in range(i + 1, n):
                    flow_value = float(tm[i, j] + tm[j, i])
                    if flow_value > 0.0:
                        pair_rows.append((flow_value, i + 1, j + 1))
            pair_rows.sort(key=lambda item: item[0], reverse=True)
            top_rows = pair_rows[: min(10, len(pair_rows))]
            total_top_flow = float(sum(row[0] for row in top_rows))
            if total_top_flow > 0.0:
                cross_flow = 0.0
                adjacent_flow = 0.0
                for flow_value, left_facility, right_facility in top_rows:
                    left_bay = facility_to_bay.get(int(left_facility))
                    right_bay = facility_to_bay.get(int(right_facility))
                    if left_bay is None or right_bay is None:
                        continue
                    if left_bay != right_bay:
                        cross_flow += float(flow_value)
                    if abs(int(left_bay) - int(right_bay)) == 1:
                        adjacent_flow += float(flow_value)
                top_flow_cross_ratio = cross_flow / total_top_flow
                top_flow_adjacent_ratio = adjacent_flow / total_top_flow

        action_history_features = []
        eval_norm = float(max(1, self.action_budget_heavy_cap))
        for action_idx in self.all_action_ids:
            history = self._standard46_action_history(action_idx)
            action_history_features.extend(
                [
                    min(float(history["candidate_rate"]), 1.0),
                    min(float(history["final_rate"]), 1.0),
                    min(float(history["avg_eval_count"]) / eval_norm, 1.0),
                ]
            )
        ...
        global_extra = [
            ...
            max_bay_violation,
            mean_bay_violation,
            top_flow_cross_ratio,
            top_flow_adjacent_ratio,
            min(float(getattr(self, "_last_reward_eval_cost_penalty", 0.0)), 1.0),
        ]
        return np.asarray(global_extra + action_history_features, dtype=np.float32)
    ...
    def _current_phase_action_ids(self, solution):
        if self._current_action_mode(solution) == "infeasible":
            return list(self.infeasible_phase_action_ids["repair"])
        return self.dqn_operator_dispatcher.current_phase_action_ids(self, solution)

    def _get_allowed_action_indices(self, solution):
        current_action_ids = self._current_action_ids(solution)
    ...
    def _compose_training_reward(self, reward_components):
        base_reward = self.dqn_reward_engine.compose_training_reward(self, reward_components)
        eval_count = max(0, int(getattr(self, "_last_action_eval_count", 0)))
        eval_penalty = float(self.reward_eval_cost_penalty) * float(max(0, eval_count - 1))
        self._last_reward_eval_cost_penalty = float(eval_penalty)
        return float(base_reward - eval_penalty)
        ...
        while attempts < max_attempts:
            ...
            for recipe in self.bootstrap_recipes:
                if attempts >= max_attempts:
                    break
                candidate = self._generate_standard46_action_sequence_candidate(base_solution, recipe)
                attempts += 1
    ...
    def _build_evaluated_candidate_from_layout(self, base_solution, permutation, bay):
        ...
        candidate.fbs_model.permutation = perm_list
        candidate.fbs_model.bay = bay_list
        self._standard46_current_action_eval_count = int(
            getattr(self, "_standard46_current_action_eval_count", 0)
        ) + 1
        self._evaluate_solution(candidate)
        return candidate

    def _clone_evaluated_candidate(self, solution):
        candidate = copy.deepcopy(solution)
        self._standard46_current_action_eval_count = int(
            getattr(self, "_standard46_current_action_eval_count", 0)
        ) + 1
    ...
    def _standard46_bay_structure(self, solution):
        return [
            [int(facility) for facility in current_bay]
            for current_bay in permutationToArray(
    ...
    def _standard46_layout_key(self, structure):
        return tuple(tuple(int(facility) for facility in current_bay) for current_bay in structure)

    def _standard46_validate_structure(self, structure, expected_facilities):
        if not structure or any(len(current_bay) == 0 for current_bay in structure):
            return False
        flat = [int(facility) for current_bay in structure for facility in current_bay]
        return sorted(flat) == sorted(int(facility) for facility in expected_facilities)

    def _standard46_candidate_cap_limit(self):
        return max(
            1,
            int(
                getattr(
                    self,
                    "_standard46_current_candidate_cap",
                    getattr(self, "new_action_candidate_cap", 6),
                )
            ),
        )

    def _standard46_action_history(self, action_idx):
        stats = self.action_telemetry.get(int(action_idx), {})
        selected = max(0, int(stats.get("selected", 0)))
        if selected <= 0:
            return {
                "selected": 0,
                "candidate_rate": 0.0,
        ...
        return {
            "selected": selected,
            "candidate_rate": float(stats.get("candidate_improved", 0)) / float(selected),
            "final_rate": float(stats.get("final_improved", 0)) / float(selected),
            "gbest_rate": float(stats.get("global_best_hits", 0)) / float(selected),
            "avg_eval_count": float(stats.get("eval_count_sum", 0.0)) / float(selected),
        }

    def _standard46_budget_level_for_cap(self, cap):
        cap = int(cap)
        if cap >= int(self.action_budget_heavy_cap):
            return "heavy"
        if cap >= int(self.action_budget_medium_cap):
            return "medium"
        return "light"

    def _standard46_select_action_budget(self, solution, action_idx, phase="main"):
        action_idx = int(action_idx)
        phase = str(phase or "main")
        light_cap = int(self.action_budget_light_cap)
        medium_cap = int(self.action_budget_medium_cap)
        heavy_cap = int(self.action_budget_heavy_cap)

        if not self.action_budget_controller_enabled:
            cap = min(int(self.new_action_candidate_cap), heavy_cap)
            return max(1, cap), self._standard46_budget_level_for_cap(cap)

        if phase in {"elite", "mid_shot"}:
            cap = medium_cap
            if action_idx in {3, 5} and self.no_improve_steps >= self.action_budget_heavy_no_improve:
                cap = heavy_cap
            return cap, self._standard46_budget_level_for_cap(cap)

        if phase != "main":
            return light_cap, "light"

        if int(getattr(solution, "current_d_inf", 0)) > 0:
            if action_idx == 100:
                cap = medium_cap
            elif action_idx in {101, 102}:
                cap = max(light_cap, medium_cap - 1)
            else:
                cap = light_cap
            return cap, self._standard46_budget_level_for_cap(cap)
        ...
        final_rate = float(history["final_rate"])
        candidate_rate = float(history["candidate_rate"])
        gbest_rate = float(history["gbest_rate"])

        cap = light_cap
        if (
            selected >= int(self.action_budget_min_selected)
            and self.no_improve_steps >= int(self.action_budget_medium_no_improve)
            and (final_rate >= self.action_budget_medium_min_final_rate or candidate_rate >= 0.04)
        ):
            cap = medium_cap
        if (
            selected >= int(self.action_budget_min_selected) * 2
            and self.no_improve_steps >= int(self.action_budget_heavy_no_improve)
            and action_idx in {3, 5}
            and (final_rate >= self.action_budget_heavy_min_final_rate or gbest_rate > 0.0)
        ):
            cap = heavy_cap
        return cap, self._standard46_budget_level_for_cap(cap)

    def _standard46_add_structure_candidate(self, base_solution, candidates, seen, structure):
        if len(candidates) >= self._standard46_candidate_cap_limit():
            return
        base_structure = self._standard46_bay_structure(base_solution)
        expected = [facility for current_bay in base_structure for facility in current_bay]
        cleaned = [[int(facility) for facility in current_bay] for current_bay in structure if len(current_bay) > 0]
        if not self._standard46_validate_structure(cleaned, expected):
            return
        key = self._standard46_layout_key(cleaned)
        if key in seen or key == self._standard46_layout_key(base_structure):
            return
        seen.add(key)
        candidate_perm, candidate_bay = arrayToPermutation(cleaned)
        candidate = self._build_evaluated_candidate_from_layout(
            base_solution,
            candidate_perm,
            candidate_bay,
        )
    ...
    def _standard46_add_perm_candidate(self, base_solution, candidates, seen, permutation, bay=None):
        if len(candidates) >= self._standard46_candidate_cap_limit():
            return
        base_perm = self._to_int_list(base_solution.fbs_model.permutation)
        base_bay = self._to_int_list(base_solution.fbs_model.bay)
        perm_list = self._to_int_list(permutation)
        bay_list = self._to_int_list(base_bay if bay is None else bay)
        if len(perm_list) != len(base_perm) or len(bay_list) != len(base_bay):
            return
        if sorted(perm_list) != sorted(base_perm):
            return
        bay_list[-1] = 1
        key = (tuple(perm_list), tuple(bay_list))
        base_key = (tuple(base_perm), tuple(base_bay))
    ...
    def _standard46_select_best_candidate(self, solution, candidates):
        if not candidates:
            return self._clone_evaluated_candidate(solution)
        return min(candidates, key=self._candidate_rank_key)

    def _standard46_flow_matrix(self, solution):
        flow = np.asarray(solution.F, dtype=float)
        if flow.ndim != 2:
            return np.zeros((0, 0), dtype=float)
        symmetric = flow + flow.T
        np.fill_diagonal(symmetric, 0.0)
        return symmetric

    def _standard46_facility_positions(self, structure):
        positions = {}
        flat_position = 0
        for bay_idx, current_bay in enumerate(structure):
            for pos_idx, facility in enumerate(current_bay):
                positions[int(facility)] = (int(bay_idx), int(pos_idx), int(flat_position))
                flat_position += 1
        return positions

    def _standard46_facility_load_order(self, solution, limit=12):
        tm = np.asarray(getattr(solution, "TM", []), dtype=float)
        if tm.ndim != 2 or tm.shape[0] == 0:
            return []
        load = np.sum(tm, axis=0) + np.sum(tm, axis=1)
        ordered = np.argsort(load)[::-1]
        return [int(idx) + 1 for idx in ordered[: max(1, int(limit))] if np.isfinite(load[int(idx)])]

    def _standard46_affinity_order(self, flow, facility, candidates, limit=4):
        facility_idx = int(facility) - 1
        scored = []
        for other in candidates:
            other_idx = int(other) - 1
            if 0 <= facility_idx < flow.shape[0] and 0 <= other_idx < flow.shape[1]:
                score = float(flow[facility_idx, other_idx])
            else:
                score = 0.0
            scored.append((score, int(other)))
        scored.sort(key=lambda item: item[0], reverse=True)
        return [facility for score, facility in scored[: max(1, int(limit))] if score > 0.0]

    def _standard46_bay_pair_flow(self, flow, left, right):
        total = 0.0
        for left_facility in left:
            left_idx = int(left_facility) - 1
            if left_idx < 0 or left_idx >= flow.shape[0]:
                continue
            for right_facility in right:
                right_idx = int(right_facility) - 1
                if 0 <= right_idx < flow.shape[1]:
                    total += float(flow[left_idx, right_idx])
        return float(total)

    def _standard46_light_proxy_context(self, solution, structure, flow):
        n = sum(len(current_bay) for current_bay in structure)
        ...
        finite_aspect_limits = aspect_limits[np.isfinite(aspect_limits) & (aspect_limits > 0.0)]
        ...
        flow_scale = float(np.sum(np.triu(flow, 1))) if flow.ndim == 2 else 0.0
        return {
            "areas": area_array,
            "aspect_limits": aspect_limits,
            "default_aspect_limit": default_aspect_limit,
            "target_bay_area": total_area / float(max(1, len(structure))),
            "avg_bay_size": float(max(1, n)) / float(max(1, len(structure))),
            "flow_scale": max(flow_scale, 1.0),
            "H": max(float(getattr(solution, "H", 1.0)), 1e-8),
        }

    def _standard46_light_structure_penalty(self, context, structure):
        area_array = context["areas"]
        target_area = max(float(context["target_bay_area"]), 1e-8)
        H_value = max(float(context["H"]), 1e-8)
        avg_bay_size = max(float(context["avg_bay_size"]), 1.0)
        area_penalties = []
        geometry_pressures = []
        for current_bay in structure:
            bay_area = self._compute_bay_area_proxy(current_bay, area_array)
            area_penalties.append(abs(float(bay_area) - target_area) / target_area)
            geometry_pressures.append(
                self._compute_bay_geometry_pressure_proxy(
                    current_bay,
                    bay_area,
                    area_array,
                    context["aspect_limits"],
                    float(context["default_aspect_limit"]),
                    H_value,
                    avg_bay_size,
                )
            )
        area_penalty = float(np.mean(area_penalties)) if area_penalties else 0.0
        geometry_penalty = float(np.mean(geometry_pressures)) if geometry_pressures else 0.0
        return float(area_penalty + 0.50 * geometry_penalty)

    def _standard46_light_geometry_pressure(self, context, structure):
        area_array = context["areas"]
        H_value = max(float(context["H"]), 1e-8)
        avg_bay_size = max(float(context["avg_bay_size"]), 1.0)
        pressures = []
    ...
    def _standard46_light_d_inf_proxy(self, context, structure):
        area_array = context["areas"]
        ...
        for current_bay in structure:
            bay_area = self._compute_bay_area_proxy(current_bay, area_array)
            if bay_area <= 0.0:
                d_inf += len(current_bay)
                continue
            bay_width = max(float(bay_area) / H_value, 1e-8)
            for facility in current_bay:
                ...
                facility_length = area_value / bay_width
                short_side = min(facility_length, bay_width)
                long_side = max(facility_length, bay_width)
                aspect_limit = np.inf
                ...
                lower_bound = math.sqrt(area_value / aspect_limit) if np.isfinite(aspect_limit) else 0.0
                upper_bound = math.sqrt(area_value * aspect_limit) if np.isfinite(aspect_limit) else float("inf")
                short_violation = max(0.0, lower_bound - short_side)
                long_violation = max(0.0, long_side - upper_bound)
                if short_violation > 0.0 or long_violation > 0.0:
                    d_inf += 1
                    violation_sum += short_violation + long_violation
        return int(d_inf), float(violation_sum)

    def _standard46_ranked_insert_positions_by_affinity(self, flow, moved, target_bay, limit=3):
        moved_list = [int(facility) for facility in moved]
        if not target_bay:
            return [0]
        bay_support = self._standard46_bay_pair_flow(flow, moved_list, target_bay)
        scored = []
        for insert_idx in range(len(target_bay) + 1):
            score = 0.05 * bay_support
            if insert_idx > 0:
                score += self._standard46_bay_pair_flow(flow, moved_list, [target_bay[insert_idx - 1]])
            if insert_idx < len(target_bay):
                score += self._standard46_bay_pair_flow(flow, moved_list, [target_bay[insert_idx]])
            scored.append((float(score), int(insert_idx)))
        scored.sort(key=lambda item: (item[0], -abs(item[1] - len(target_bay) / 2.0)), reverse=True)
        return [insert_idx for _score, insert_idx in scored[: max(1, int(limit))]]

    def _standard46_insert_positions_near_affinity(self, flow, moved, target_bay, limit=4):
        moved_list = [int(facility) for facility in moved]
        if not target_bay:
            return [0]
        scored = []
        for pos_idx, facility in enumerate(target_bay):
            score = 0.0
            facility_idx = int(facility) - 1
            for moved_facility in moved_list:
                moved_idx = int(moved_facility) - 1
                if 0 <= moved_idx < flow.shape[0] and 0 <= facility_idx < flow.shape[1]:
                    score += float(flow[moved_idx, facility_idx])
            scored.append((score, int(pos_idx)))
        scored.sort(key=lambda item: item[0], reverse=True)
        positions = {0, len(target_bay)}
        for score, pos_idx in scored[: max(1, int(limit))]:
            if score <= 0.0:
                continue
            positions.add(pos_idx)
            positions.add(pos_idx + 1)
        return sorted(pos for pos in positions if 0 <= pos <= len(target_bay))

    def _standard46_random_pair_positions(self, n, count):
        pairs = []
        if n < 2:
            return pairs
    ...
    def _generate_new_facility_pair_swap_candidate(self, solution):
        base_perm = self._to_int_list(solution.fbs_model.permutation)
        base_bay = self._to_int_list(solution.fbs_model.bay)
        n = len(base_perm)
        candidates = []
        seen = set()
        if n < 2:
        ...
        position_by_facility = {int(facility): idx for idx, facility in enumerate(base_perm)}
        swap_positions = set()
        for i, j, _pair_mhc in self._build_topk_pair_rows(solution)[: min(14, n)]:
            facility_a = int(i) + 1
            facility_b = int(j) + 1
            if facility_a in position_by_facility and facility_b in position_by_facility:
                swap_positions.add((position_by_facility[facility_a], position_by_facility[facility_b]))
        load_order = self._standard46_facility_load_order(solution, limit=min(10, n))
        flow = self._standard46_flow_matrix(solution)
        for facility in load_order:
            partners = self._standard46_affinity_order(
                flow,
                facility,
                [other for other in base_perm if int(other) != int(facility)],
                limit=3,
            )
            for partner in partners:
                swap_positions.add((position_by_facility[int(facility)], position_by_facility[int(partner)]))
        swap_positions.update(self._standard46_random_pair_positions(n, 4))

        for i, j in swap_positions:
            if i == j:
                continue
            candidate_perm = list(base_perm)
            candidate_perm[int(i)], candidate_perm[int(j)] = candidate_perm[int(j)], candidate_perm[int(i)]
            self._standard46_add_perm_candidate(solution, candidates, seen, candidate_perm, base_bay)
        return self._standard46_select_best_candidate(solution, candidates)

    def _generate_new_intra_bay_facility_insert_candidate(self, solution):
        structure = self._standard46_bay_structure(solution)
        positions = self._standard46_facility_positions(structure)
        flow = self._standard46_flow_matrix(solution)
        candidates = []
        seen = set()
        source_facilities = self._standard46_facility_load_order(solution, limit=14)
        for facility in source_facilities:
            if facility not in positions:
                continue
            bay_idx, pos_idx, _flat = positions[facility]
            current_bay = list(structure[bay_idx])
            if len(current_bay) < 2:
                continue
            remaining = current_bay[:pos_idx] + current_bay[pos_idx + 1 :]
            partners = self._standard46_affinity_order(flow, facility, remaining, limit=4)
            insert_positions = {0, len(remaining)}
            for partner in partners:
                partner_pos = remaining.index(partner)
                insert_positions.update({partner_pos, partner_pos + 1})
            for insert_idx in sorted(insert_positions):
                if insert_idx == pos_idx:
                    continue
                candidate_bay = remaining[:insert_idx] + [facility] + remaining[insert_idx:]
                candidate_structure = [list(item) for item in structure]
                candidate_structure[bay_idx] = candidate_bay
                self._standard46_add_structure_candidate(solution, candidates, seen, candidate_structure)

        for bay_idx, current_bay in enumerate(structure):
            if len(current_bay) < 2:
                continue
            for _ in range(2):
                source_idx, insert_idx = np.random.choice(len(current_bay), 2, replace=False)
                facility = current_bay[int(source_idx)]
                remaining = current_bay[: int(source_idx)] + current_bay[int(source_idx) + 1 :]
                insert_idx = min(int(insert_idx), len(remaining))
                candidate_structure = [list(item) for item in structure]
                candidate_structure[bay_idx] = remaining[:insert_idx] + [facility] + remaining[insert_idx:]
                self._standard46_add_structure_candidate(solution, candidates, seen, candidate_structure)
        return self._standard46_select_best_candidate(solution, candidates)

    def _standard46_move_bay(self, structure, source_idx, insert_idx):
        candidate = [list(item) for item in structure]
        moved = candidate.pop(int(source_idx))
        target_idx = int(insert_idx)
        if int(source_idx) < target_idx:
            target_idx -= 1
        target_idx = min(max(target_idx, 0), len(candidate))
        candidate.insert(target_idx, moved)
        return candidate

    def _generate_new_bay_reorder_candidate(self, solution):
        structure = self._standard46_bay_structure(solution)
        if len(structure) < 2:
            return self._clone_evaluated_candidate(solution)
        flow = self._standard46_flow_matrix(solution)
        ...
        for i in range(len(structure)):
            for j in range(i + 1, len(structure)):
                score = self._standard46_bay_pair_flow(flow, structure[i], structure[j])
                pair_scores.append((score, i, j))
        pair_scores.sort(key=lambda item: item[0], reverse=True)
        for score, i, j in pair_scores[: min(8, len(pair_scores))]:
            if score <= 0.0:
                continue
            candidate_structure = [list(item) for item in structure]
            candidate_structure[i], candidate_structure[j] = candidate_structure[j], candidate_structure[i]
            self._standard46_add_structure_candidate(solution, candidates, seen, candidate_structure)
            if abs(i - j) > 1:
                self._standard46_add_structure_candidate(
                    solution,
                    candidates,
                    seen,
                    self._standard46_move_bay(structure, i, j),
                )
                self._standard46_add_structure_candidate(
                    solution,
                    candidates,
                    seen,
                    self._standard46_move_bay(structure, j, i + 1),
                )
        for _ in range(4):
            i, j = np.random.choice(len(structure), 2, replace=False)
            candidate_structure = [list(item) for item in structure]
            candidate_structure[int(i)], candidate_structure[int(j)] = candidate_structure[int(j)], candidate_structure[int(i)]
            self._standard46_add_structure_candidate(solution, candidates, seen, candidate_structure)
        return self._standard46_select_best_candidate(solution, candidates)

    def _generate_new_intra_bay_segment_insert_candidate(self, solution):
        structure = self._standard46_bay_structure(solution)
        positions = self._standard46_facility_positions(structure)
        flow = self._standard46_flow_matrix(solution)
        segment_lengths = tuple(int(length) for length in getattr(self, "segment_insert_light_segment_lengths", (2, 3, 4)))
        candidates = []
        seen = set()
        focus_facilities = self._standard46_facility_load_order(solution, limit=12)
        for facility in focus_facilities:
            if facility not in positions:
                continue
            bay_idx, pos_idx, _flat = positions[facility]
            current_bay = list(structure[bay_idx])
            for segment_len in segment_lengths:
                if len(current_bay) <= segment_len:
                    continue
                start_min = max(0, pos_idx - segment_len + 1)
                start_max = min(pos_idx, len(current_bay) - segment_len)
                for start_idx in range(start_min, start_max + 1):
                    ...
                    insert_positions = set(
                        self._standard46_insert_positions_near_affinity(flow, segment, remaining, limit=4)
                    )
                    for insert_idx in sorted(insert_positions):
        ...
        bay_candidates = [idx for idx, current_bay in enumerate(structure) if len(current_bay) > 3]
        for _ in range(4):
            if not bay_candidates:
                break
            bay_idx = int(np.random.choice(np.asarray(bay_candidates, dtype=int)))
            current_bay = list(structure[bay_idx])
            valid_lengths = [length for length in segment_lengths if len(current_bay) > length]
            if not valid_lengths:
                continue
            segment_len = int(np.random.choice(np.asarray(valid_lengths, dtype=int)))
            start_idx = int(np.random.randint(0, len(current_bay) - segment_len + 1))
            segment = current_bay[start_idx : start_idx + segment_len]
            remaining = current_bay[:start_idx] + current_bay[start_idx + segment_len :]
            insert_idx = int(np.random.randint(0, len(remaining) + 1))
            candidate_structure = [list(item) for item in structure]
            candidate_structure[bay_idx] = remaining[:insert_idx] + segment + remaining[insert_idx:]
            self._standard46_add_structure_candidate(solution, candidates, seen, candidate_structure)
        return self._standard46_select_best_candidate(solution, candidates)
    ...
    def _generate_new_guarded_cross_bay_relocate_light_candidate(self, solution, structure, flow):
        ...
        base_geometry_pressure = self._standard46_light_geometry_pressure(context, structure)
        base_d_inf_proxy, base_violation_proxy = self._standard46_light_d_inf_proxy(context, structure)
        base_is_feasible = bool(getattr(solution, "current_is_feasible", False))
        base_key = self._standard46_layout_key(structure)
        scored_facilities = []
        for facility, (source_bay_idx, _pos_idx, _flat) in positions.items():
            source_bay = list(structure[source_bay_idx])
            if len(source_bay) <= 1:
                continue
            source_remaining = [item for item in source_bay if int(item) != int(facility)]
            source_support = self._standard46_bay_pair_flow(flow, [facility], source_remaining)
            cross_support = 0.0
            for target_bay_idx, target_bay in enumerate(structure):
                if target_bay_idx == source_bay_idx:
                    continue
                cross_support += self._standard46_bay_pair_flow(flow, [facility], target_bay)
            scored_facilities.append((cross_support - source_support, int(facility)))
        scored_facilities.sort(key=lambda item: item[0], reverse=True)

        best_score = -float("inf")
        best_structure = None
        for _facility_score, facility in scored_facilities[: min(16, len(scored_facilities))]:
            source_bay_idx, pos_idx, _flat = positions[facility]
            source_bay = list(structure[source_bay_idx])
            if len(source_bay) <= 1:
                continue
            block = [int(facility)]
            reduced_source = source_bay[:pos_idx] + source_bay[pos_idx + 1 :]
            source_support = self._standard46_bay_pair_flow(flow, block, reduced_source)
            target_scores = []
            for target_bay_idx, target_bay in enumerate(structure):
                if target_bay_idx == source_bay_idx:
                    continue
                target_scores.append(
                    (
                        self._standard46_bay_pair_flow(flow, block, target_bay),
                        int(target_bay_idx),
                    )
                )
            target_scores.sort(key=lambda item: item[0], reverse=True)
            for target_support, target_bay_idx in target_scores[: min(5, len(target_scores))]:
                target_bay = list(structure[target_bay_idx])
                for insert_idx in self._standard46_ranked_insert_positions_by_affinity(
                    flow,
                    block,
                    target_bay,
                    limit=2,
                    ...
                    candidate_structure = [list(item) for item in structure]
                    candidate_structure[source_bay_idx] = reduced_source
                    target_copy = list(candidate_structure[target_bay_idx])
                    target_copy = target_copy[:insert_idx] + block + target_copy[insert_idx:]
                    candidate_structure[target_bay_idx] = target_copy
                    if self._standard46_layout_key(candidate_structure) == base_key:
                        continue
                    candidate_penalty = self._standard46_light_structure_penalty(context, candidate_structure)
                    candidate_geometry_pressure = self._standard46_light_geometry_pressure(context, candidate_structure)
                    candidate_d_inf_proxy, candidate_violation_proxy = self._standard46_light_d_inf_proxy(
                        context,
                        candidate_structure,
                    )
                    ...
                    if (
                        ...
                        and geometry_worse > 0.0
                    ):
                        continue
                    support_gain = float(target_support - source_support) / max(
                        float(target_support + source_support),
                        1.0,
                    )
                    bay_distance_penalty = 0.04 * abs(target_bay_idx - source_bay_idx) / max(
                        1,
                        len(structure) - 1,
                    )
                    score = (
                        support_gain
                        + 0.75 * penalty_gain
                        - 0.80 * max(0.0, geometry_worse)
                        - 0.20 * max(0, d_inf_worse)
                        - 0.05 * max(0.0, violation_worse)
                        - bay_distance_penalty
                    )
                    if score > best_score:
    ...
    def _generate_new_cross_bay_relocate_candidate(self, solution):
        ...
        if len(structure) < 2:
            return self._clone_evaluated_candidate(solution)
        flow = self._standard46_flow_matrix(solution)
        if self._standard46_candidate_cap_limit() <= 1:
            return self._generate_new_guarded_cross_bay_relocate_light_candidate(solution, structure, flow)
        positions = self._standard46_facility_positions(structure)
        tm = np.asarray(getattr(solution, "TM", []), dtype=float)
        candidates = []
        seen = set()
        scored_facilities = []
        for facility, (bay_idx, _pos_idx, _flat) in positions.items():
            if len(structure[bay_idx]) <= 1:
                continue
            facility_idx = int(facility) - 1
            cross_score = 0.0
            if tm.ndim == 2 and 0 <= facility_idx < tm.shape[0]:
                for other, (other_bay_idx, _other_pos, _other_flat) in positions.items():
                    if other_bay_idx == bay_idx:
                        continue
                    other_idx = int(other) - 1
                    if 0 <= other_idx < tm.shape[1]:
                        cross_score += float(tm[facility_idx, other_idx] + tm[other_idx, facility_idx])
            scored_facilities.append((cross_score, int(facility)))
        scored_facilities.sort(key=lambda item: item[0], reverse=True)

        for _score, facility in scored_facilities[: min(12, len(scored_facilities))]:
            source_bay_idx, pos_idx, _flat = positions[facility]
            source_bay = list(structure[source_bay_idx])
            block_starts = [(pos_idx, 1)]
            if len(source_bay) > 2:
                if pos_idx > 0:
                    block_starts.append((pos_idx - 1, 2))
                if pos_idx + 1 < len(source_bay):
                    block_starts.append((pos_idx, 2))
            for start_idx, block_len in block_starts:
                block = source_bay[start_idx : start_idx + block_len]
                if len(source_bay) <= len(block):
                    continue
                reduced_source = source_bay[:start_idx] + source_bay[start_idx + block_len :]
                target_scores = []
                for target_bay_idx, target_bay in enumerate(structure):
                    if target_bay_idx == source_bay_idx:
                        continue
                ...
                target_scores.sort(key=lambda item: item[0], reverse=True)
                for target_score, target_bay_idx in target_scores[: min(4, len(target_scores))]:
                    if target_score <= 0.0:
                        continue
                    target_bay = list(structure[target_bay_idx])
                    for insert_idx in self._standard46_insert_positions_near_affinity(flow, block, target_bay, limit=4):
                        candidate_structure = [list(item) for item in structure]
                        candidate_structure[source_bay_idx] = reduced_source
                        target_copy = list(candidate_structure[target_bay_idx])
                        target_copy = target_copy[:insert_idx] + block + target_copy[insert_idx:]
    ...
    def _standard46_boundary_structures(self, structure, flow, targeted_pairs=None):
        if len(structure) < 2:
            return []
        pair_scores = []
        for bay_idx in range(len(structure) - 1):
            if targeted_pairs is not None and bay_idx not in targeted_pairs:
                continue
            score = self._standard46_bay_pair_flow(flow, structure[bay_idx], structure[bay_idx + 1])
            pair_scores.append((score, bay_idx))
        pair_scores.sort(key=lambda item: item[0], reverse=True)
        generated = []
        for _score, bay_idx in pair_scores[: min(4, len(pair_scores))]:
            left = list(structure[bay_idx])
            right = list(structure[bay_idx + 1])
            merged = left + right
            if len(merged) < 2:
                continue
            current_split = len(left)
            split_candidates = set(range(max(1, current_split - 2), min(len(merged), current_split + 3)))
            if len(merged) <= 8:
                split_candidates.update(range(1, len(merged)))
            for split_idx in sorted(split_candidates):
                if split_idx <= 0 or split_idx >= len(merged) or split_idx == current_split:
                    continue
                candidate_structure = [list(item) for item in structure]
                candidate_structure[bay_idx] = merged[:split_idx]
                candidate_structure[bay_idx + 1] = merged[split_idx:]
                generated.append(candidate_structure)
            for left_size in range(1, min(2, len(left)) + 1):
                for right_size in range(1, min(2, len(right)) + 1):
                    new_left = left[:-left_size] + right[:right_size]
                    new_right = left[-left_size:] + right[right_size:]
                    if not new_left or not new_right:
                        continue
                    candidate_structure = [list(item) for item in structure]
                    candidate_structure[bay_idx] = new_left
                    candidate_structure[bay_idx + 1] = new_right
                generated.append(candidate_structure)
        return generated
    ...
    def _generate_new_boundary_shift_light_candidate(self, solution, structure, flow):
        ...
        for bay_idx in range(len(structure) - 1):
            pair_scores.append(
                (
                    self._standard46_bay_pair_flow(flow, structure[bay_idx], structure[bay_idx + 1]),
                    int(bay_idx),
                )
            )
        pair_scores.sort(key=lambda item: item[0], reverse=True)
        ...
        best_score = -float("inf")
        best_structure = None
        for current_cut, bay_idx in pair_scores:
            left = list(structure[bay_idx])
            right = list(structure[bay_idx + 1])
            variants = []
            if len(left) > 1:
                moved = [left[-1]]
                variants.append((left[:-1], moved + right, moved, left[:-1], right))
            if len(right) > 1:
                moved = [right[0]]
                variants.append((left + moved, right[1:], moved, right[1:], left))
            for candidate_left, candidate_right, moved, source_remaining, target_bay in variants:
                candidate_structure = [list(item) for item in structure]
                candidate_structure[bay_idx] = candidate_left
                candidate_structure[bay_idx + 1] = candidate_right
                if self._standard46_layout_key(candidate_structure) == base_key:
                    continue
                candidate_penalty = self._standard46_light_structure_penalty(context, candidate_structure)
                candidate_geometry_pressure = self._standard46_light_geometry_pressure(context, candidate_structure)
                ...
                target_support = self._standard46_bay_pair_flow(flow, moved, target_bay)
                support_gain = float(target_support - source_support) / max(
                    float(target_support + source_support),
                    1.0,
                )
    ...
    def _generate_new_adjacent_boundary_block_repartition_candidate(self, solution):
        ...
        flow = self._standard46_flow_matrix(solution)
        if self._standard46_candidate_cap_limit() <= 1:
            return self._generate_new_boundary_shift_light_candidate(solution, structure, flow)
        candidates = []
        seen = set()
        for candidate_structure in self._standard46_boundary_structures(structure, flow):
            self._standard46_add_structure_candidate(solution, candidates, seen, candidate_structure)
        return self._standard46_select_best_candidate(solution, candidates)

    def _standard46_bay_count_structures(self, structure, flow, targeted_bays=None):
        generated = []
        split_candidates = []
        for bay_idx, current_bay in enumerate(structure):
            if targeted_bays is not None and bay_idx not in targeted_bays:
                continue
            if len(current_bay) < 2:
                continue
            for split_idx in range(1, len(current_bay)):
                ...
                score = self._standard46_bay_pair_flow(flow, left, right)
                split_candidates.append((score, bay_idx, split_idx))
        split_candidates.sort(key=lambda item: item[0])
        for _score, bay_idx, split_idx in split_candidates[: min(8, len(split_candidates))]:
            current_bay = list(structure[bay_idx])
            candidate_structure = [list(item) for item in structure]
            candidate_structure[bay_idx] = current_bay[:split_idx]
            candidate_structure.insert(bay_idx + 1, current_bay[split_idx:])
            generated.append(candidate_structure)

        merge_candidates = []
        for bay_idx in range(len(structure) - 1):
            if targeted_bays is not None and bay_idx not in targeted_bays and bay_idx + 1 not in targeted_bays:
                continue
            score = self._standard46_bay_pair_flow(flow, structure[bay_idx], structure[bay_idx + 1])
            merge_candidates.append((score, bay_idx))
        merge_candidates.sort(key=lambda item: item[0], reverse=True)
        for _score, bay_idx in merge_candidates[: min(8, len(merge_candidates))]:
            candidate_structure = [list(item) for item in structure]
            candidate_structure[bay_idx] = list(structure[bay_idx]) + list(structure[bay_idx + 1])
            candidate_structure.pop(bay_idx + 1)
            generated.append(candidate_structure)
        return generated

    def _generate_new_bay_count_adjust_candidate(self, solution):
        structure = self._standard46_bay_structure(solution)
        flow = self._standard46_flow_matrix(solution)
        candidates = []
        seen = set()
        for candidate_structure in self._standard46_bay_count_structures(structure, flow):
            self._standard46_add_structure_candidate(solution, candidates, seen, candidate_structure)
        return self._standard46_select_best_candidate(solution, candidates)

    def _generate_new_geometry_repair_candidate(self, solution):
        current = self._clone_evaluated_candidate(solution)
        previous_cap = int(getattr(self, "new_action_candidate_cap", 6))
        previous_current_cap = int(self._standard46_candidate_cap_limit())
        self.new_action_candidate_cap = max(previous_cap, 32)
        self._standard46_current_candidate_cap = max(previous_current_cap, 32)
        max_rounds = max(3, min(6, int(getattr(self, "problem_size", 60)) // 8))
        try:
            for _round_idx in range(max_rounds):
                if getattr(current, "current_is_feasible", False):
                    break
                structure = self._standard46_bay_structure(current)
                flow = self._standard46_flow_matrix(current)
                infeasible_mask = np.asarray(getattr(current, "infeasible_mask", []), dtype=bool).reshape(-1)
                targeted_bays = set()
                if infeasible_mask.size > 0:
                    positions = self._standard46_facility_positions(structure)
                    for facility, (bay_idx, _pos_idx, _flat) in positions.items():
                        mask_idx = int(facility) - 1
                        if 0 <= mask_idx < infeasible_mask.size and bool(infeasible_mask[mask_idx]):
                            targeted_bays.add(int(bay_idx))
                if not targeted_bays:
                    targeted_bays = set(range(len(structure)))

                targeted_pairs = set()
                for bay_idx in targeted_bays:
                    if bay_idx > 0:
                        targeted_pairs.add(bay_idx - 1)
                    if bay_idx < len(structure) - 1:
                        targeted_pairs.add(bay_idx)

                candidates = []
                seen = set()
                for candidate_structure in self._standard46_bay_count_structures(
                    structure,
                    flow,
                    targeted_bays=targeted_bays,
                ):
                    self._standard46_add_structure_candidate(current, candidates, seen, candidate_structure)
                for candidate_structure in self._standard46_boundary_structures(
                    structure,
                    flow,
                ...
                if not candidates:
                    break
                best_candidate = min(candidates, key=self._layout_score)
                if self._layout_score(best_candidate) >= self._layout_score(current):
                    break
            ...
            self.new_action_candidate_cap = previous_cap
            self._standard46_current_candidate_cap = previous_current_cap

    def _generate_standard46_new_action_candidate_impl(self, solution, action_idx, phase="main"):
        action_idx = int(action_idx)
        if action_idx == 0:
            return self._generate_new_facility_pair_swap_candidate(solution)
        if action_idx == 1:
            return self._generate_new_intra_bay_facility_insert_candidate(solution)
        if action_idx == 2:
            return self._generate_new_bay_reorder_candidate(solution)
        if action_idx == 3:
            return self._generate_new_intra_bay_segment_insert_candidate(solution)
        if action_idx == 4:
            return self._generate_new_cross_bay_relocate_candidate(solution)
        if action_idx == 5:
            return self._generate_new_adjacent_boundary_block_repartition_candidate(solution)
        if action_idx == 100:
            return self._generate_new_geometry_repair_candidate(solution)
        if action_idx == 101:
            return self._generate_new_bay_count_adjust_candidate(solution)
        if action_idx == 102:
            return self._generate_new_adjacent_boundary_block_repartition_candidate(solution)
        raise ValueError(f"Unsupported Standard4.6 rewritten action id: {action_idx}")

    def _generate_standard46_new_action_candidate(self, solution, action_idx, phase="main"):
        action_idx = int(action_idx)
        budget_cap, budget_level = self._standard46_select_action_budget(solution, action_idx, phase=phase)
        previous_cap = int(getattr(self, "_standard46_current_candidate_cap", self.new_action_candidate_cap))
        previous_eval_count = int(getattr(self, "_standard46_current_action_eval_count", 0))
        self._standard46_current_candidate_cap = max(1, int(budget_cap))
        self._standard46_current_action_eval_count = 0
        candidate = None
        try:
            candidate = self._generate_standard46_new_action_candidate_impl(
                solution,
                action_idx,
                phase=phase,
            )
            eval_count = int(getattr(self, "_standard46_current_action_eval_count", 0))
        finally:
            self._standard46_current_candidate_cap = previous_cap
            self._standard46_current_action_eval_count = previous_eval_count

        self._last_action_eval_count = max(0, int(eval_count))
        self._last_action_budget_cap = max(1, int(budget_cap))
        self._last_action_budget_level = str(budget_level)
        if candidate is not None:
            candidate.standard46_action_eval_count = int(self._last_action_eval_count)
            candidate.standard46_action_budget_cap = int(self._last_action_budget_cap)
            candidate.standard46_action_budget_level = str(self._last_action_budget_level)
        return candidate

    def _generate_standard46_action_sequence_candidate(self, solution, action_ids):
        candidate = copy.deepcopy(solution)
        self._evaluate_solution(candidate)
        for action_idx in action_ids:
            candidate = self._generate_standard46_new_action_candidate(candidate, int(action_idx), phase="sequence")
        return candidate
    ...
    def _prepare_episode_start(self, episode_idx):
        ...
        if recipe is None:
            return
        candidate = self._generate_standard46_action_sequence_candidate(self.s, recipe)
        if candidate.current_is_feasible:
            self.s = candidate
            self.current_energy = self.s.fitness
            self._observe_feasible_state(self.s)
    ...
    def _attempt_diversification(self, global_step, force=False):
        if (not force) and (global_step - self.last_diversify_step < max(10, self.t_max // 4)):
            return
        candidate = self._generate_standard46_action_sequence_candidate(self.s, self.diversify_recipe)
