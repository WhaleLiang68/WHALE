...
PAPER_OBJECTIVE_DEFINITION_VERSION = "paper_mhc_f3_proxy_v1"
...
class ELP(StandardELP):
    ...
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0, archive_limit=64, objective_weights=None):
        ...
        self.paper_instance_profile = copy.deepcopy(
            getattr(base_env, "paper_instance_profile", MO_PaperInstanceProfileUtil.build_metadata(base_env))
        )
        self.mo_weights = np.asarray(
            objective_weights if objective_weights is not None else [0.5, 0.5, 0.0, 0.0],
            dtype=float,
        ).reshape(-1)
        if self.mo_weights.size < 4:
            self.mo_weights = np.pad(self.mo_weights, (0, 4 - self.mo_weights.size), constant_values=0.0)
        self.mo_weights = self.mo_weights[:4]
        if not np.any(self.mo_weights > 0):
            self.mo_weights = np.asarray([0.5, 0.5, 0.0, 0.0], dtype=float)
        ...
        self.paper_final_window_ratio = float(os.getenv("ELP_PAPER_FINAL_WINDOW_RATIO", "0.10"))
        if not (0.0 < self.paper_final_window_ratio <= 1.0):
            raise ValueError("ELP_PAPER_FINAL_WINDOW_RATIO 必须位于 (0, 1] 区间。")
        self.paper_final_window_start_progress = 1.0 - self.paper_final_window_ratio
        self.paper_solution_set_mode = "terminal_window"
        self._paper_collect_terminal_population = False
        self.current_progress_ratio = 0.0
        self._paper_total_steps = 0
        self.paper_observed_objectives_raw = []
        self.paper_observed_solution_keys = set()
        self.paper_final_objectives_raw = []
        self.paper_final_solution_keys = set()
        self.paper_preference_matrix = None
        self.paper_preference_payload = None
        self.paper_area_utilization = None
        ...
        self.paper_preference_matrix, self.paper_preference_payload = MO_PaperPreferenceUtil.load_or_generate_matrix(
            facility_count,
            instance_name=self.instance_name,
        )
        total_area = float(np.sum(np.asarray(getattr(base_env, "areas", []), dtype=float)))
        facility_width = float(getattr(base_env, "W", 0.0) or 0.0)
        facility_height = float(getattr(base_env, "H", 0.0) or 0.0)
        facility_area = facility_width * facility_height
        self.paper_area_utilization = None if facility_area <= 0.0 else float(total_area / facility_area)
    ...
    def _compute_reference_front_metrics(self):
        paper_metrics = self._compute_paper_metrics()
        return {
            "archive_hypervolume": None,
            "archive_spacing": None,
            "archive_igd": None,
            "reference_front_path": None,
            "reference_front_size": None,
            "reference_front_archive_count": None,
            "archive_hypervolume_mode": None,
            "archive_hypervolume_reference_point": None,
            **paper_metrics,
        }

    def _compute_paper_metrics(self):
        final_summary = MO_PaperMetricsUtil.calculate_summary(self.paper_final_objectives_raw)
        history_summary = MO_PaperMetricsUtil.calculate_summary(self.paper_observed_objectives_raw)
        return {
            **final_summary,
            "paper_solution_set_mode": self.paper_solution_set_mode,
            "paper_solution_set_detail": (
                f"last_{self.paper_final_window_ratio:.2f}_run_window"
                if self.paper_solution_set_mode == "terminal_window"
                else "final_population"
            ),
            "paper_final_window_ratio": float(self.paper_final_window_ratio),
            "paper_final_window_start_progress": float(self.paper_final_window_start_progress),
            "paper_final_window_empty": bool(final_summary["paper_solution_count"] == 0),
            "paper_history_solution_count": history_summary["paper_solution_count"],
            "paper_history_pareto_count": history_summary["paper_pareto_count"],
        }
    ...
    def _extra_training_progress_text(self):
        ...
        paper_metrics = self._compute_paper_metrics()
        return (
            f" | PR_final: {_fmt_metric(paper_metrics['paper_pr'])}"
            f" | SP_raw_final: {_fmt_metric(paper_metrics['paper_sp_raw'])}"
            f" | SP_norm_final: {_fmt_metric(paper_metrics['paper_sp_norm'])}"
            f" | OPS_final: {_fmt_metric(paper_metrics['paper_ops'])}"
        )
    ...
    def _solution_trace_fields(self, solution, prefix):
        ...
        return {
            ...
            f"{prefix}F3": self._safe_float(getattr(solution, "F3", None)),
    ...
    def _score_candidate_encoding(self, permutation, bay, solution):
        ...
        objectives_raw = MO_FBSUtil.calculate_objectives(
            ...
            metrics["mhc"],
            len(metrics["fac_x"]),
            preference_matrix=self.paper_preference_matrix,
            area_utilization=1.0 if self.paper_area_utilization is None else self.paper_area_utilization,
        )
    ...
    def _sync_solution_metrics(self, solution, metrics):
        ...
        solution.F3 = float(metrics["f3"])
        solution.CR = float(metrics["f3"])
    ...
    def _observe_feasible_state(self, solution):
        ...
        raw_objectives = np.asarray(getattr(solution, "mo_objectives_raw", []), dtype=float).reshape(-1)
        if raw_objectives.size >= 2 and np.all(np.isfinite(raw_objectives[:2])):
            key = self._paper_solution_signature(solution)
            if key not in self.paper_observed_solution_keys:
                self.paper_observed_solution_keys.add(key)
                self.paper_observed_objectives_raw.append(raw_objectives[:2].copy())
            if self._is_in_paper_final_window() and key not in self.paper_final_solution_keys:
                self.paper_final_solution_keys.add(key)
                self.paper_final_objectives_raw.append(raw_objectives[:2].copy())
    ...
    def _archive_item_payload(self, solution, index):
        ...
        return {
            ...
            "f3Proxy": float(getattr(solution, "F3", getattr(solution, "CR", 0.0))),
    ...
    def _save_pareto_archive(self, start_time, algorithm_name=None):
        ...
        payload = {
            "instance": self.instance_name,
            "algorithm": algo_tag,
            "objectiveDefinitionVersion": PAPER_OBJECTIVE_DEFINITION_VERSION,
            "paperPreferenceVersion": None if self.paper_preference_payload is None else self.paper_preference_payload.get("version"),
            "paperPreferenceSeed": None if self.paper_preference_payload is None else self.paper_preference_payload.get("seed"),
            "paperAreaUtilization": self.paper_area_utilization,
            **copy.deepcopy(self.paper_instance_profile),
    ...
    @staticmethod
    def _paper_solution_signature(solution):
        ...
        raw_objectives = np.asarray(getattr(solution, "mo_objectives_raw", []), dtype=float).reshape(-1)
        return ("objectives", tuple(float(value) for value in np.round(raw_objectives[:2], decimals=12)))

    def _is_in_paper_final_window(self):
        if self.paper_solution_set_mode == "final_population":
            return bool(getattr(self, "_paper_collect_terminal_population", False))
        progress_ratio = min(max(float(getattr(self, "current_progress_ratio", 0.0) or 0.0), 0.0), 1.0)
        total_steps = int(max(0, getattr(self, "_paper_total_steps", 0) or 0))
        if total_steps > 0:
            observed_step = int(max(getattr(self, "_trace_global_step", 0) or 0, 0))
            # 观测发生在当前动作执行之后，末段判定要按“已完成步数”计。
            observed_progress = min(max(float(observed_step + 1) / float(total_steps), 0.0), 1.0)
            progress_ratio = max(progress_ratio, observed_progress)
        return progress_ratio >= float(self.paper_final_window_start_progress)
    ...
    def _run_pso_baseline(
        ...
        self.paper_solution_set_mode = "final_population"
        ...
        for generation in range(generations):
            self._paper_collect_terminal_population = generation >= generations - 1
            self.current_progress_ratio = (
                1.0 if generations <= 1 else float(generation) / float(generations - 1)
            )
        ...
        self.last_run_payload = {
            **copy.deepcopy(self.paper_instance_profile),
            ...
            "rep_f3": None if best_solution is None else float(getattr(best_solution, "F3", 0.0)),
            ...
            "paper_solution_count": reference_metrics["paper_solution_count"],
            "paper_pareto_count": reference_metrics["paper_pareto_count"],
            "paper_solution_set_mode": reference_metrics["paper_solution_set_mode"],
            "paper_solution_set_detail": reference_metrics["paper_solution_set_detail"],
            "paper_pr": reference_metrics["paper_pr"],
            "paper_sp": reference_metrics["paper_sp"],
            "paper_sp_raw": reference_metrics["paper_sp_raw"],
            "paper_sp_norm": reference_metrics["paper_sp_norm"],
            "paper_ops": reference_metrics["paper_ops"],
            "paper_ops_components": reference_metrics["paper_ops_components"],
            "paper_best_mhc": reference_metrics["paper_best_mhc"],
            "paper_mean_mhc": reference_metrics["paper_mean_mhc"],
            "paper_best_f3": reference_metrics["paper_best_f3"],
            "paper_mean_f3": reference_metrics["paper_mean_f3"],
            "paper_final_window_ratio": reference_metrics["paper_final_window_ratio"],
            "paper_final_window_start_progress": reference_metrics["paper_final_window_start_progress"],
            "paper_final_window_empty": reference_metrics["paper_final_window_empty"],
            "paper_history_solution_count": reference_metrics["paper_history_solution_count"],
            "paper_history_pareto_count": reference_metrics["paper_history_pareto_count"],
            "mo_run_id": None,
            "mo_bundle_dir": None,
            "mo_trace_path": None,
            "mo_events_path": None,
    ...
    def run_moea_baseline(
        ...
        best_observed = self.best_feasible_cost
        self.current_progress_ratio = 1.0
        self._paper_collect_terminal_population = True
    ...
    def _run_impl(self):
        ...
        run_algorithm = os.getenv("ELP_EXP_ALGORITHM", "ELP_DRL_MO4_PAPER")
        ...
        self._paper_total_steps = int(total_steps)
        ...
        run_summary = {
            ...
            "repF3": None if best_solution is None else self._safe_float(getattr(best_solution, "F3", None)),
            ...
            "paperAreaUtilization": self.paper_area_utilization,
            **copy.deepcopy(self.paper_instance_profile),
            "paperSolutionCount": reference_metrics["paper_solution_count"],
            "paperParetoCount": reference_metrics["paper_pareto_count"],
            "paperSolutionSetMode": reference_metrics["paper_solution_set_mode"],
            "paperSolutionSetDetail": reference_metrics["paper_solution_set_detail"],
            "paperPr": reference_metrics["paper_pr"],
            "paperSp": reference_metrics["paper_sp"],
            "paperSpRaw": reference_metrics["paper_sp_raw"],
            "paperSpNorm": reference_metrics["paper_sp_norm"],
            "paperOps": reference_metrics["paper_ops"],
            "paperOpsComponents": reference_metrics["paper_ops_components"],
            "paperBestMhc": reference_metrics["paper_best_mhc"],
            "paperMeanMhc": reference_metrics["paper_mean_mhc"],
            "paperBestF3": reference_metrics["paper_best_f3"],
            "paperMeanF3": reference_metrics["paper_mean_f3"],
            "paperFinalWindowRatio": reference_metrics["paper_final_window_ratio"],
            "paperFinalWindowStartProgress": reference_metrics["paper_final_window_start_progress"],
            "paperFinalWindowEmpty": reference_metrics["paper_final_window_empty"],
            "paperHistorySolutionCount": reference_metrics["paper_history_solution_count"],
            "paperHistoryParetoCount": reference_metrics["paper_history_pareto_count"],
...
if __name__ == "__main__":
    def _format_summary_metrics(solver, best_energy):
        ...
        paper_pr = payload.get("paper_pr")
        paper_sp_raw = payload.get("paper_sp_raw")
        paper_sp_norm = payload.get("paper_sp_norm")
        ...
        return (
            ...
            f"PR: {_fmt(paper_pr)} | SP_raw: {_fmt(paper_sp_raw)} | "
            f"SP_norm: {_fmt(paper_sp_norm)} | OPS: {_fmt(paper_ops)}"
        )
    ...
    default_algorithm = f"MO_BASELINE_{baseline_algo.upper()}_PAPER" if baseline_enabled else "ELP_DRL_MO4_PAPER"
    exp_algorithm = os.getenv("ELP_EXP_ALGORITHM", default_algorithm)
    default_remark = (
        "MO baseline on action-sequence encoding under paper metrics"
        if baseline_enabled
        else "MO4 search skeleton under paper MHC + f3_proxy objectives and PR/SP/OPS metrics"
    )
    ...
    def _run_once(run_index):
        ...
        try:
            ...
            env.reset()
        base_env = env.unwrapped if hasattr(env, "unwrapped") else env
        paper_profile = MO_PaperInstanceProfileUtil.apply_to_env(base_env)
        initial_gbest = copy.deepcopy(base_env)
        logger.info(
            "Paper instance profile | "
            f"flow_source: {paper_profile['paperFlowSource']} | "
            f"override: {paper_profile['paperFlowOverrideApplied']} | "
            f"constraint_note: {paper_profile['paperConstraintProfileNote'] or 'none'}"
        )
