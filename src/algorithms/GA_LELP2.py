...
class Q_LearningLELP:
    ...
    def _generate_new_solution(self, s,op):
        ...
        next_state, _, _, _ = new_s.step(op)
        return new_s
...
if __name__ == "__main__":
    ...
    exp_algorithm = "LELP2_with_PopOpt"  # 算法名称改为ELP
    exp_remark = "agent更新在外循环的外面"
    exp_number = 30
    is_exp = True
    ...
    if is_exp:
        ...
        for i in range(exp_number):
            ...
            try:
                ...
                pop_best_solution = pop_optimizer.optimize()  # 现在只返回FBSModel对象
                print(f"pop_best_solution的内存地址: {id(pop_best_solution)}")
                # 单独计算种群优化结果的适应度
                env.reset(fbs_model=pop_best_solution)
                pop_best_fitness = env.fitness  # 从环境中获取适应度值
                logger.info(f"种群优化完成，初始最优适应度: {pop_best_fitness}")
                if env.fbs_model == pop_best_solution:
                    print("pop_best_solution与self.env相同")
                else:
                    print("pop_best_solution与self.env不相同")
                initial_gbest = copy.deepcopy(env)  # 初始gbest为环境初始解
                if initial_gbest.fbs_model == pop_best_solution:
                    print("pop_best_solution与gbest相同")
                else:
                    print("pop_best_solution与gbest不相同")
                # 2. 实例化ELP算法（传入图片要求的所有输入参数）
                elp_solver = Q_LearningLELP(
                    env=env,
                    gbest=initial_gbest,
                    T=T_initial,
                    Q_matrix=Q_matrix,
