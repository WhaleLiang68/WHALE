...
class ELP:
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0):
        ...
        self.valid_actions = [0, 2, 3, 6, 9, 10, 11, 12, 13]
        ...
        self._consecutive_cross_bay = 0
        self._consecutive_segment_insert = 0
    ...
    def state_encoder(self, solution):
        ...
        h_band = self._histogram_band(solution.fitness)
        state = d_band
        for band in (gap_band, s_band, h_band):
            state = state * 4 + band
        return state
    ...
    def _compute_transition_reward(
        ...
        reward = 0.0
        scale = max(abs(float(self.best_feasible_cost or 1.0)), 1.0)
        ...
        if previous_d_inf > 0 and next_d_inf == 0:
            reward += 0.05
        elif next_d_inf < previous_d_inf:
            reward += 0.01 * (previous_d_inf - next_d_inf)

        if np.isfinite(next_cost) and next_cost < previous_best_feasible:
            gap = (previous_best_feasible - next_cost) / scale
            reward += min(gap * 3.0, 0.15)
    ...
    def _prepare_episode_start(self, episode_idx):
        ...
        self.energy_histogram = {
            k: v * 0.5
            for k, v in self.energy_histogram.items()
            if v * 0.5 > 0.5
        }
    ...
    def _run_impl(self):
        ...
        agent = StandardQLearningAgent(s_dim=256, a_dim=len(self.valid_actions))
        prior = {
            0: 0.05,  # facility_swap
            2: 0.05,  # bay_swap
            6: 0.03,  # bay_shuffle
            9: 0.06,  # flow_guided_swap
            10: 0.08,  # segment_insert
            11: 0.07,  # cross_bay_relocate
            12: 0.06,  # bay_split_by_flow
            13: 0.06,  # bay_merge_by_flow
        }
        for table_idx, action_idx in enumerate(self.valid_actions):
            agent.Q[:, table_idx] = prior.get(action_idx, 0.03)
        ...
        for episode in range(self.G):
            ...
            for _ in range(self.t_max):
                ...
                if real_action_idx == 11 and self._consecutive_cross_bay >= 3:
                    allowed_without_cross = [
                        i for i in allowed_actions if self.valid_actions[i] != 11
                    ]
                    if allowed_without_cross:
                        action_table_idx = agent.select_action(
                            current_state_idx, allowed_actions=allowed_without_cross
                        )
                        real_action_idx = self.valid_actions[action_table_idx]
                if real_action_idx == 10 and self._consecutive_segment_insert >= 8:
                    allowed_without_segment = [
                        i for i in allowed_actions if self.valid_actions[i] != 10
                    ]
                    if allowed_without_segment:
                        action_table_idx = agent.select_action(
                            current_state_idx, allowed_actions=allowed_without_segment
                        )
                        real_action_idx = self.valid_actions[action_table_idx]
                self._consecutive_cross_bay = (
                    self._consecutive_cross_bay + 1 if real_action_idx == 11 else 0
                )
                self._consecutive_segment_insert = (
                    self._consecutive_segment_insert + 1 if real_action_idx == 10 else 0
                )
                previous_cost = self.s.fitness
                previous_d_inf = self.s.current_d_inf
...
if __name__ == "__main__":
    ...
    exp_remark = "ELP+full-Q-learning,移除了 bay_flip、facility_insert (5) 和 facility_shuffle (7)+先验质量设初始值+h衰减+奖励函数修改"
    exp_number = 50
    is_exp = True
