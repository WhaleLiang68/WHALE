...
class ELP:
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0):
        ...
        self.enable_eval_cache = True
        self.eval_cache_maxsize = 2048
    ...
    def _clone_model(model):
        ...
        cloned_model.permutation = list(getattr(model, "permutation", []))
        cloned_model.bay = list(getattr(model, "bay", []))
        genes = getattr(model, "genes", None)
        cloned_model.genes = [] if genes is None else list(genes)
        return cloned_model

    def _clone_solution(self, solution):
        cloned = copy.copy(solution)
        cloned.fbs_model = self._clone_model(getattr(solution, "fbs_model", None))
        return cloned
    ...
    def _normalize_v_worst_for_cache(self, v_worst):
        if v_worst is None:
            return None
        value = float(v_worst)
    ...
    def _build_eval_cache_key(self, fbs_model, v_worst):
        return (
            tuple(fbs_model.permutation),
            tuple(fbs_model.bay),
            self._normalize_v_worst_for_cache(v_worst),
            int(self.k_penalty),
            "manhattan",
        )

    def _get_cached_metrics(self, key):
        if (not self.enable_eval_cache) or self.eval_cache_maxsize <= 0:
            return None
        metrics = self._eval_cache.get(key)
        if metrics is None:
    ...
    def _put_cached_metrics(self, key, metrics):
        if (not self.enable_eval_cache) or self.eval_cache_maxsize <= 0:
            return
        self._eval_cache[key] = metrics
        self._eval_cache.move_to_end(key)
        if len(self._eval_cache) > self.eval_cache_maxsize:
            self._eval_cache.popitem(last=False)

    def _evaluate_layout_metrics(self, fbs_model, v_worst):
        key = self._build_eval_cache_key(fbs_model, v_worst)
        cached = self._get_cached_metrics(key)
        ...
        metrics = FBSUtil.evaluate_layout(
            ...
            self.s.H,
            self.s.F,
            self.s.aspect_limits,
            v_worst=v_worst,
            k_penalty=self.k_penalty,
            distance_metric="manhattan",
        )
        self._put_cached_metrics(key, metrics)
        return metrics

    def _evaluate_layout_cost(self, fbs_model, v_worst):
        metrics = self._evaluate_layout_metrics(fbs_model, v_worst)
        return float(metrics["cost"])

    def _evaluate_solution(self, solution):
        metrics = self._evaluate_layout_metrics(solution.fbs_model, self.worst_feasible_cost)
    ...
    def _sample_temperature_floor(self, n_samples=None, target_accept=None):
        ...
        base_solution = self._clone_solution(self.best_feasible_solution)
        allowed_actions = [
            table_idx
        ...
        for _ in range(max(1, n_samples)):
            ...
            candidate = self._clone_solution(base_solution)
            recipe = self.action_recipes[action_idx]
            self._apply_recipe(candidate, recipe)
            candidate_cost = self._evaluate_layout_cost(candidate.fbs_model, local_v_worst)
    ...
    def _sample_initial_temperature_from_neighborhood(self, base_solution, sample_size=None, quantile=None):
        ...
        # Fill remaining samples with random allowed actions if needed
        if len(deltas) < sample_size:
            allowed_actions = self._get_allowed_action_indices(base_solution)
            if allowed_actions:
                for _ in range(sample_size - len(deltas)):
                    action_table_idx = int(np.random.choice(allowed_actions))
                    action_idx = self.valid_actions[action_table_idx]
                    candidate = self.generate_candidate_by_action(base_solution, action_idx)
                    if np.isfinite(candidate.fitness):
                        delta = abs(float(candidate.fitness) - base_cost)
                        if delta > 0:
                            deltas.append(delta)
    ...
    def generate_candidate_by_action(self, solution, action_idx):
        candidate = self._clone_solution(solution)
        recipe = self.action_recipes[action_idx]
        self._apply_recipe(candidate, recipe)
        self._evaluate_solution(candidate)
        return candidate
...
if __name__ == "__main__":
    ...
    exp_remark = "WarmStart(GA)+ELP+full-Q-learning,移除了 facility_insert (5) 和 facility_shuffle (7)，6的加速版本"
    exp_number = 50
    is_exp = True
