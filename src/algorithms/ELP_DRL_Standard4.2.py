...
class ELP:
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0):
        ...
        self.two_stage_learned_evaluator_collect_data = _env_flag(
            ...
            bool(_runtime_default("two_stage_learned_evaluator_collect_data", True)),
        )
        self.two_stage_learned_evaluator_collect_full_labels = _env_flag(
            "ELP_TWO_STAGE_LEARNED_EVALUATOR_COLLECT_FULL_LABELS",
            bool(_runtime_default("two_stage_learned_evaluator_collect_full_labels", True)),
        )
        ...
        self.two_stage_action14_exact_eval_enabled = _env_flag(
            "ELP_TWO_STAGE_ACTION14_EXACT_EVAL_ENABLED",
            bool(_runtime_default("two_stage_action14_exact_eval_enabled", True)),
        )
        self.two_stage_action14_reorder_block_cap = max(
            1,
            int(_runtime_default("two_stage_action14_reorder_block_cap", 2)),
        )
        self.two_stage_action14_reorder_variants_per_pair = max(
            0,
            int(_runtime_default("two_stage_action14_reorder_variants_per_pair", 1)),
        )
        self.two_stage_action14_exact_probe_topk = max(
            0,
            _env_int(
                "ELP_TWO_STAGE_ACTION14_EXACT_PROBE_TOPK",
                int(_runtime_default("two_stage_action14_exact_probe_topk", 4)),
            ),
        )
        self.two_stage_action14_exact_select_by_post_ls_probe = _env_flag(
            "ELP_TWO_STAGE_ACTION14_EXACT_SELECT_BY_POST_LS_PROBE",
            bool(_runtime_default("two_stage_action14_exact_select_by_post_ls_probe", False)),
        )
        self.two_stage_learned_survivor_diversity_enabled = bool(
            _runtime_default("two_stage_learned_survivor_diversity_enabled", True)
        )
        self.two_stage_learned_survivor_diversity_weight = min(
            max(float(_runtime_default("two_stage_learned_survivor_diversity_weight", 0.22)), 0.0),
            1.0,
        )
        self.two_stage_learned_survivor_diversity_template_bonus = min(
            max(float(_runtime_default("two_stage_learned_survivor_diversity_template_bonus", 0.10)), 0.0),
            0.50,
        )
        ...
        self.two_stage_telemetry = {
            action_idx: {
                ...
                "exact_probe_calls": 0,
                "exact_probe_switches": 0,
                "exact_probe_topk_sum": 0,
                "exact_probe_best_rank_sum": 0.0,
                "exact_probe_immediate_final_delta_sum": 0.0,
                "exact_probe_probe_final_delta_sum": 0.0,
                "exact_probe_final_uplift_sum": 0.0,
                "exact_probe_ls_triggered": 0,
                "exact_probe_ls_improved": 0,
                "exact_probe_selected_source_counts": {},
                "exact_probe_selected_template_counts": {},
            }
        ...
        logger.info(
            ...
            f"action14_exact: {self.two_stage_action14_exact_eval_enabled} | "
            f"action14_probe: topk={self.two_stage_action14_exact_probe_topk}/post_ls={self.two_stage_action14_exact_select_by_post_ls_probe} | "
            f"survivor_diversity: {self.two_stage_learned_survivor_diversity_enabled}/w={self.two_stage_learned_survivor_diversity_weight:.2f} | "
            f"collect_data: {self.two_stage_learned_evaluator_collect_data} | "
            f"collect_full_labels: {self.two_stage_learned_evaluator_collect_full_labels} | "
            f"dataset_path: {self.two_stage_learned_evaluator_dataset_path}"
        )
    ...
    def format_action_telemetry(self):
        ...
        if self.two_stage_heavy_actions_enabled and self.two_stage_telemetry:
            ...
            for action_idx in sorted(self.two_stage_telemetry):
                ...
                exact_probe_suffix = ""
                exact_probe_calls = int(stats.get("exact_probe_calls", 0))
                if exact_probe_calls > 0:
                    probe_templates = self._format_two_stage_template_counter(
                        stats.get("exact_probe_selected_template_counts", {}),
                        topn=3,
                    )
                    probe_sources = self._format_two_stage_template_counter(
                        stats.get("exact_probe_selected_source_counts", {}),
                        topn=3,
                    )
                    avg_probe_topk = float(stats.get("exact_probe_topk_sum", 0.0)) / float(exact_probe_calls)
                    avg_probe_rank = float(stats.get("exact_probe_best_rank_sum", 0.0)) / float(exact_probe_calls)
                    avg_selected_final = float(stats.get("exact_probe_immediate_final_delta_sum", 0.0)) / float(
                        exact_probe_calls
                    )
                    avg_probe_final = float(stats.get("exact_probe_probe_final_delta_sum", 0.0)) / float(
                        exact_probe_calls
                    )
                    avg_probe_uplift = float(stats.get("exact_probe_final_uplift_sum", 0.0)) / float(
                        exact_probe_calls
                    )
                    exact_probe_suffix = (
                        " | exact_probe="
                        f"topk:{avg_probe_topk:.2f} calls:{exact_probe_calls} "
                        f"switches:{int(stats.get('exact_probe_switches', 0))} "
                        f"avg_probe_rank:{avg_probe_rank:.2f} "
                        f"avg_selected_final:{avg_selected_final:.2f} "
                        f"avg_probe_final:{avg_probe_final:.2f} "
                        f"uplift:{avg_probe_uplift:+.2f} "
                        f"ls:{int(stats.get('exact_probe_ls_triggered', 0))}/{int(stats.get('exact_probe_ls_improved', 0))} "
                        f"probe_sources={probe_sources} probe_top={probe_templates}"
                    )
                lines.append(
                    (
                        ...
                        "| top_survivors={top_survivors} | top_selected={top_selected}{exact_probe_suffix}"
                    ).format(
                        idx=int(action_idx),
                        ...
                        exact_probe_suffix=exact_probe_suffix,
                )
            )
        return lines
    ...
    @staticmethod
    def _two_stage_jaccard_ratio(left_values, right_values):
        left_set = {str(item) for item in left_values if str(item)}
        right_set = {str(item) for item in right_values if str(item)}
    ...
    def _build_two_stage_candidate_signature(self, context, candidate_entry):
        permutation = self._to_int_list(candidate_entry.get("permutation", []))
        bay = self._to_int_list(candidate_entry.get("bay", []))
        if len(permutation) == 0 or len(permutation) != len(bay):
            return None

        base_positions = context.get("positions", {})
        base_facility_to_bay = context.get("facility_to_bay", {})
        base_bay = self._to_int_list(context.get("bay", []))
        if len(base_bay) != len(bay):
            return None

        bay = list(bay)
        bay[-1] = 1
        candidate_boundary_positions = {
            int(idx)
            for idx, flag in enumerate(bay[:-1])
            if int(flag) == 1
        }
        base_boundary_positions = {
            int(idx)
            for idx, flag in enumerate(base_bay[:-1])
            if int(flag) == 1
        }

        changed_position_facilities = set()
        changed_bay_facilities = set()
        current_bay_idx = 0
        for pos, facility in enumerate(permutation):
            facility = int(facility)
            if int(base_positions.get(facility, -1)) != int(pos):
                changed_position_facilities.add(str(facility))
            if int(base_facility_to_bay.get(facility, -1)) != int(current_bay_idx):
                changed_bay_facilities.add(str(facility))
            if int(bay[pos]) == 1:
                current_bay_idx += 1

        changed_facilities = changed_position_facilities | changed_bay_facilities
        template_name = self._two_stage_template_name(candidate_entry.get("meta"))
        return {
            "template_name": str(template_name),
            "changed_facilities": tuple(sorted(changed_facilities)),
            "changed_bay_facilities": tuple(sorted(changed_bay_facilities)),
            "changed_boundary_positions": tuple(
                sorted(candidate_boundary_positions.symmetric_difference(base_boundary_positions))
            ),
        }

    def _two_stage_candidate_similarity(self, left_signature, right_signature):
        if left_signature is None or right_signature is None:
            return 0.0

        facility_overlap = self._two_stage_jaccard_ratio(
            left_signature.get("changed_facilities", ()),
            right_signature.get("changed_facilities", ()),
        )
        bay_overlap = self._two_stage_jaccard_ratio(
            left_signature.get("changed_bay_facilities", ()),
            right_signature.get("changed_bay_facilities", ()),
        )
        boundary_overlap = self._two_stage_jaccard_ratio(
            left_signature.get("changed_boundary_positions", ()),
            right_signature.get("changed_boundary_positions", ()),
        )
        similarity = (
            0.50 * float(facility_overlap)
            + 0.30 * float(bay_overlap)
            + 0.20 * float(boundary_overlap)
        )
        if (
            str(left_signature.get("template_name", ""))
            and str(left_signature.get("template_name", "")) == str(right_signature.get("template_name", ""))
        ):
            similarity += float(self.two_stage_learned_survivor_diversity_template_bonus)
        return min(max(float(similarity), 0.0), 1.0)

    @staticmethod
    def _constraint_violation_contrib(short_side, long_side, lower_bounds, upper_bounds):
        short_violation = np.maximum(np.asarray(lower_bounds, dtype=float) - np.asarray(short_side, dtype=float), 0.0)
        long_violation = np.maximum(np.asarray(long_side, dtype=float) - np.asarray(upper_bounds, dtype=float), 0.0)
        return short_violation + long_violation

    def _evaluate_action14_candidate_exact(self, base_solution, context, candidate_entry):
        meta = candidate_entry.get("meta")
        if not isinstance(meta, dict):
            return None
        pair_idx = int(meta.get("pair_idx", -1))
        split_idx = int(meta.get("split_idx", -1))
        bay_structure = context.get("bay_structure", [])
        if pair_idx < 0 or pair_idx + 1 >= len(bay_structure):
            return None

        merged_sequence = meta.get("merged_sequence")
        if isinstance(merged_sequence, (list, tuple)):
            merged_sequence = [int(facility) for facility in merged_sequence]
        else:
            base_left = [int(facility) for facility in bay_structure[pair_idx]]
            base_right = [int(facility) for facility in bay_structure[pair_idx + 1]]
            merged_sequence = base_left + base_right
        if len(merged_sequence) < 2 or not (0 < split_idx < len(merged_sequence)):
            return None

        candidate_permutation = self._to_int_list(candidate_entry.get("permutation", []))
        candidate_bay = self._to_int_list(candidate_entry.get("bay", []))
        if candidate_permutation and candidate_bay and len(candidate_permutation) != len(candidate_bay):
            return None

        affected_indices = np.asarray(merged_sequence, dtype=int) - 1
        if affected_indices.size == 0:
            return None

        areas = np.asarray(context.get("areas", []), dtype=float)
        H_value = max(float(context.get("H", 1.0)), 1e-8)
        pair_areas = areas[affected_indices]
        left_indices = affected_indices[:split_idx]
        right_indices = affected_indices[split_idx:]
        if left_indices.size == 0 or right_indices.size == 0:
            return None

        left_area = float(np.sum(pair_areas[:split_idx]))
        right_area = float(np.sum(pair_areas[split_idx:]))
        left_width = left_area / H_value
        right_width = right_area / H_value
        if left_width <= 0.0 or right_width <= 0.0:
            return None

        base_fac_x = np.asarray(base_solution.fac_x, dtype=float)
        base_fac_y = np.asarray(base_solution.fac_y, dtype=float)
        base_fac_b = np.asarray(base_solution.fac_b, dtype=float)
        base_fac_h = np.asarray(base_solution.fac_h, dtype=float)
        full_fac_x = base_fac_x.copy()
        full_fac_y = base_fac_y.copy()
        full_fac_b = base_fac_b.copy()
        full_fac_h = base_fac_h.copy()

        bay_x_offsets = np.asarray(context.get("bay_x_offsets", []), dtype=float)
        if pair_idx >= bay_x_offsets.size:
            return None
        x_offset_left = float(bay_x_offsets[pair_idx])
        x_offset_right = float(x_offset_left + left_width)

        left_lengths = pair_areas[:split_idx] / left_width
        right_lengths = pair_areas[split_idx:] / right_width
        left_y = np.cumsum(left_lengths, dtype=float) - left_lengths * 0.5
        right_y = np.cumsum(right_lengths, dtype=float) - right_lengths * 0.5

        full_fac_x[left_indices] = x_offset_left + left_width * 0.5
        full_fac_y[left_indices] = left_y
        full_fac_b[left_indices] = left_lengths
        full_fac_h[left_indices] = left_width

        full_fac_x[right_indices] = x_offset_right + right_width * 0.5
        full_fac_y[right_indices] = right_y
        full_fac_b[right_indices] = right_lengths
        full_fac_h[right_indices] = right_width

        lower_bounds = np.asarray(base_solution.lower_bounds, dtype=float)
        upper_bounds = np.asarray(base_solution.upper_bounds, dtype=float)
        base_infeasible_mask = np.asarray(base_solution.infeasible_mask, dtype=bool)
        short_new = np.minimum(full_fac_b[affected_indices], full_fac_h[affected_indices])
        long_new = np.maximum(full_fac_b[affected_indices], full_fac_h[affected_indices])
        new_infeasible_affected = (short_new < lower_bounds[affected_indices]) | (
            long_new > upper_bounds[affected_indices]
        )
        infeasible_mask = base_infeasible_mask.copy()
        infeasible_mask[affected_indices] = new_infeasible_affected
        d_inf = int(np.sum(infeasible_mask))

        base_short = np.minimum(base_fac_b[affected_indices], base_fac_h[affected_indices])
        base_long = np.maximum(base_fac_b[affected_indices], base_fac_h[affected_indices])
        base_violation_affected = self._constraint_violation_contrib(
            base_short,
            base_long,
            lower_bounds[affected_indices],
            upper_bounds[affected_indices],
        )
        new_violation_affected = self._constraint_violation_contrib(
            short_new,
            long_new,
            lower_bounds[affected_indices],
            upper_bounds[affected_indices],
        )
        violation = float(context.get("base_violation", 0.0)) - float(np.sum(base_violation_affected)) + float(
            np.sum(new_violation_affected)
        )
        violation = max(float(violation), 0.0)

        base_D = np.asarray(base_solution.D, dtype=float)
        flow_matrix = np.asarray(base_solution.F, dtype=float)
        new_rows = (
            np.abs(full_fac_x[affected_indices][:, np.newaxis] - full_fac_x[np.newaxis, :])
            + np.abs(full_fac_y[affected_indices][:, np.newaxis] - full_fac_y[np.newaxis, :])
        )
        delta_mhc = float(np.sum((new_rows - base_D[affected_indices, :]) * flow_matrix[affected_indices, :]))
        unaffected_mask = np.ones(len(full_fac_x), dtype=bool)
        unaffected_mask[affected_indices] = False
        unaffected_indices = np.flatnonzero(unaffected_mask)
        if unaffected_indices.size > 0:
            new_cols = (
                np.abs(full_fac_x[unaffected_indices][:, np.newaxis] - full_fac_x[affected_indices][np.newaxis, :])
                + np.abs(full_fac_y[unaffected_indices][:, np.newaxis] - full_fac_y[affected_indices][np.newaxis, :])
            )
            delta_mhc += float(
                np.sum(
                    (new_cols - base_D[np.ix_(unaffected_indices, affected_indices)])
                    * flow_matrix[np.ix_(unaffected_indices, affected_indices)]
                )
            )
        mhc = float(base_solution.MHC) + float(delta_mhc)
        if d_inf == 0:
            cost = float(mhc)
        elif self.worst_feasible_cost is None or not np.isfinite(self.worst_feasible_cost) or float(self.worst_feasible_cost) <= 0.0:
            cost = float("inf")
        else:
            cost = float(mhc) + float((d_inf ** int(self.k_penalty)) * float(self.worst_feasible_cost))
        is_feasible = bool(d_inf == 0)
        candidate_rank_key = (
            0 if is_feasible else 1,
            int(d_inf),
            float(cost) if np.isfinite(cost) else float("inf"),
            float(violation),
            float(mhc) if np.isfinite(mhc) else float("inf"),
        )
        return {
            "affected_indices": affected_indices.astype(int),
            "fac_x": full_fac_x,
            "fac_y": full_fac_y,
            "fac_b": full_fac_b,
            "fac_h": full_fac_h,
            ...
            "mhc": float(mhc),
            "cost": float(cost) if np.isfinite(cost) else float("inf"),
            "d_inf": int(d_inf),
    ...
    def _materialize_action14_exact_candidate(self, base_solution, candidate_entry, exact_metrics):
        if exact_metrics is None:
            return None
        candidate = copy.deepcopy(base_solution)
        candidate.fbs_model.permutation = self._to_int_list(candidate_entry.get("permutation", []))
        candidate_bay = self._to_int_list(candidate_entry.get("bay", []))
        if not candidate_bay:
            return None
        candidate_bay[-1] = 1
        candidate.fbs_model.bay = candidate_bay

        fac_x = np.asarray(exact_metrics["fac_x"], dtype=float)
        fac_y = np.asarray(exact_metrics["fac_y"], dtype=float)
        fac_b = np.asarray(exact_metrics["fac_b"], dtype=float)
        fac_h = np.asarray(exact_metrics["fac_h"], dtype=float)
        min_side = np.minimum(fac_b, fac_h)
        fac_aspect_ratio = np.divide(
            np.maximum(fac_b, fac_h),
            min_side,
            out=np.full_like(fac_b, np.inf, dtype=float),
            where=min_side > 0,
        )

        affected_indices = np.asarray(exact_metrics["affected_indices"], dtype=int)
        base_D = np.asarray(base_solution.D, dtype=float)
        D = base_D.copy()
        new_rows = (
            np.abs(fac_x[affected_indices][:, np.newaxis] - fac_x[np.newaxis, :])
            + np.abs(fac_y[affected_indices][:, np.newaxis] - fac_y[np.newaxis, :])
        )
        D[affected_indices, :] = new_rows
        D[:, affected_indices] = new_rows.T
        TM = D * np.asarray(base_solution.F, dtype=float)
        ...
        metrics = {
            ...
            "lower_bounds": np.asarray(exact_metrics["lower_bounds"], dtype=float),
            "upper_bounds": np.asarray(exact_metrics["upper_bounds"], dtype=float),
            "infeasible_mask": np.asarray(exact_metrics["infeasible_mask"], dtype=bool),
            "D": D,
            "TM": TM,
            "mhc": float(exact_metrics["mhc"]),
            "cost": float(exact_metrics["cost"]),
            "d_inf": int(exact_metrics["d_inf"]),
            "is_feasible": bool(exact_metrics["is_feasible"]),
        }
        self._sync_solution_metrics(candidate, metrics)
        return candidate

    def _probe_action14_exact_candidate_post_ls(
        self,
        base_solution,
        candidate_entry,
        exact_metrics,
        previous_cost,
        previous_d_inf,
        immediate_rank_index,
    ):
        """在不污染全局档案的前提下，估计动作14候选在局部搜索后的真实吸引力。"""
        candidate = self._materialize_action14_exact_candidate(base_solution, candidate_entry, exact_metrics)
        if candidate is None:
            return None

        immediate_cost = float(getattr(candidate, "fitness", np.inf))
        immediate_rank_key = self._candidate_rank_key(candidate)
        accepted_improved = bool(
            (
                np.isfinite(previous_cost)
                and np.isfinite(immediate_cost)
                and immediate_cost + 1e-9 < float(previous_cost)
            )
            or int(getattr(candidate, "current_d_inf", 10**9)) < int(previous_d_inf)
        )
        ...
        if trigger_local_search:
            ...
            final_candidate = self._greedy_local_search(
                copy.deepcopy(candidate),
                enable_guided=run_topk_guided,
                guided_topk=guided_topk,
                guided_max_iters=guided_max_iters,
                observe_archive=False,
            )
            local_search_improved = bool(
                getattr(final_candidate, "current_is_feasible", False)
                and np.isfinite(immediate_cost)
                and np.isfinite(getattr(final_candidate, "fitness", np.inf))
                and float(final_candidate.fitness) + 1e-9 < immediate_cost
            )

        final_cost = float(getattr(final_candidate, "fitness", np.inf))
        final_rank_key = self._candidate_rank_key(final_candidate)
        return {
            "candidate_entry": candidate_entry,
            "meta": candidate_entry.get("meta"),
            "immediate_rank_index": int(immediate_rank_index),
            "immediate_rank_key": immediate_rank_key,
            "immediate_cost": immediate_cost,
            "immediate_delta": (
                float(previous_cost) - immediate_cost
                if np.isfinite(previous_cost) and np.isfinite(immediate_cost)
                else 0.0
            ),
            ...
            "final_delta": (
                float(previous_cost) - final_cost
                if np.isfinite(previous_cost) and np.isfinite(final_cost)
                else 0.0
            ),
        }

    def _generate_two_stage_action14_exact_candidate(
        self,
        solution,
        context,
        scored_candidates,
        ...
        telemetry["stage1_survivors"] += int(len(scored_candidates))
        for candidate_entry in scored_candidates:
            candidate_entry["selected_survivor"] = True
            self._record_two_stage_template_counter(
                telemetry,
                "survivor_template_counts",
                candidate_entry["meta"],
            )
        ...
        for candidate_entry in scored_candidates:
            exact_metrics = self._evaluate_action14_candidate_exact(solution, context, candidate_entry)
            if exact_metrics is None:
                return None
            ...
            candidate_entry["candidate_rank_key"] = exact_metrics["candidate_rank_key"]
            labeled_candidates.append(candidate_entry)
            exact_metrics_cache[int(candidate_entry["proposal_idx"])] = exact_metrics
            if best_rank is None or exact_metrics["candidate_rank_key"] < best_rank:
                best_rank = exact_metrics["candidate_rank_key"]
                best_entry = candidate_entry
                best_meta = candidate_entry["meta"]
                best_metrics = exact_metrics
        ...
        if self.dqn_two_stage_evaluator.collect_data and labeled_candidates:
            ...
            for rank_index, candidate_entry in enumerate(ordered_labeled_candidates):
                ...
                candidate_entry["selected_best"] = (
                    int(candidate_entry["proposal_idx"]) == int(best_entry["proposal_idx"])
                )
                dataset_records.append(
                    {
                        ...
                        "proxy_score": (
                            float(candidate_entry["proxy_score"])
                            if candidate_entry.get("proxy_score") is not None and np.isfinite(candidate_entry["proxy_score"])
                            else None
                        ),
                        "learned_score": (
                            float(candidate_entry["learned_score"])
                            if candidate_entry.get("learned_score") is not None and np.isfinite(candidate_entry["learned_score"])
                            else None
                        ),
                        "stage1_score": float(candidate_entry.get("stage1_score", 0.0)),
                        "selected_survivor": True,
                        "selected_best": bool(candidate_entry.get("selected_best", False)),
                        "true_rank_index": int(rank_index),
                        "feasible_rank": int(feasible_rank),
                        "d_inf": int(d_inf),
                        "cost": float(cost) if np.isfinite(cost) else float("inf"),
                        "violation": float(violation) if np.isfinite(violation) else float("inf"),
                        "mhc": float(mhc) if np.isfinite(mhc) else float("inf"),
            ...
            self.dqn_two_stage_evaluator.append_records(dataset_records, logger=self.logger)

        best_candidate = self._materialize_action14_exact_candidate(solution, best_entry, best_metrics)
        if best_candidate is None:
            return None
        probe_topk = int(max(0, getattr(self, "two_stage_action14_exact_probe_topk", 0)))
        if probe_topk > 0 and labeled_candidates:
            ordered_probe_candidates = sorted(
                labeled_candidates,
                key=lambda item: item.get("candidate_rank_key", (1, 10**9, float("inf"), float("inf"), float("inf"))),
            )[:probe_topk]
            probe_results = []
            previous_cost = float(getattr(solution, "fitness", np.inf))
            previous_d_inf = int(getattr(solution, "current_d_inf", 10**9))
            for rank_index, candidate_entry in enumerate(ordered_probe_candidates, start=1):
                exact_metrics = exact_metrics_cache.get(int(candidate_entry["proposal_idx"]))
                if exact_metrics is None:
                    continue
                probe_result = self._probe_action14_exact_candidate_post_ls(
                    solution,
                    candidate_entry,
                    exact_metrics,
                    previous_cost,
            ...
            if probe_results:
                immediate_probe = probe_results[0]
                probe_best = min(probe_results, key=lambda item: item["final_rank_key"])
                telemetry["exact_probe_calls"] = int(telemetry.get("exact_probe_calls", 0)) + 1
                telemetry["exact_probe_topk_sum"] = int(telemetry.get("exact_probe_topk_sum", 0)) + int(len(probe_results))
                telemetry["exact_probe_best_rank_sum"] = float(telemetry.get("exact_probe_best_rank_sum", 0.0)) + float(
                    probe_best["immediate_rank_index"]
                )
                telemetry["exact_probe_immediate_final_delta_sum"] = float(
                    telemetry.get("exact_probe_immediate_final_delta_sum", 0.0)
                ) + float(immediate_probe["final_delta"])
                telemetry["exact_probe_probe_final_delta_sum"] = float(
                    telemetry.get("exact_probe_probe_final_delta_sum", 0.0)
                ) + float(probe_best["final_delta"])
                telemetry["exact_probe_final_uplift_sum"] = float(
                    telemetry.get("exact_probe_final_uplift_sum", 0.0)
                ) + float(probe_best["final_delta"] - immediate_probe["final_delta"])
                if probe_best["trigger_local_search"]:
                    telemetry["exact_probe_ls_triggered"] = int(telemetry.get("exact_probe_ls_triggered", 0)) + 1
                if probe_best["local_search_improved"]:
                    telemetry["exact_probe_ls_improved"] = int(telemetry.get("exact_probe_ls_improved", 0)) + 1
                self._record_two_stage_template_counter(
                    telemetry,
                    "exact_probe_selected_template_counts",
                    probe_best["meta"],
                )
                chosen_source = "immediate"
                if int(probe_best["candidate_entry"]["proposal_idx"]) != int(best_entry["proposal_idx"]):
                    telemetry["exact_probe_switches"] = int(telemetry.get("exact_probe_switches", 0)) + 1
                    chosen_source = "probe"
                ...
                if self.two_stage_action14_exact_select_by_post_ls_probe and probe_best["final_candidate"] is not None:
                    best_candidate = probe_best["final_candidate"]
                    best_entry = probe_best["candidate_entry"]
                    best_meta = best_entry.get("meta")
    ...
    def _prepare_two_stage_context(self, solution):
        ...
        bay_widths = np.asarray(bay_areas, dtype=float) / max(float(getattr(solution, "H", 1.0)), 1e-8)
        ...
        return {
            ...
            "base_violation": float(self._constraint_violation(solution)),
        }
    ...
    def _propose_two_stage_adjacent_bay_repartition_full_scan(self, context):
        proposals = []
        seen_keys = set()
        bay_structure = [list(current_bay) for current_bay in context["bay_structure"]]
        sym_flow = context["sym_flow"]
        areas = context["areas"]
        full_scan_budget = 10**9
        for pair_idx in range(max(0, len(bay_structure) - 1)):
            left_bay = list(bay_structure[pair_idx])
            right_bay = list(bay_structure[pair_idx + 1])
            ...
            variant_entries = [
                {
                    "merged_sequence": left_bay + right_bay,
                    "current_split": int(len(left_bay)),
                    "variant_kind": "base",
                }
            ]
            exchange_candidates = []
            max_block_size = int(max(1, getattr(self, "two_stage_action14_reorder_block_cap", 2)))
            left_block_cap = min(max_block_size, max(0, len(left_bay) - 1))
            right_block_cap = min(max_block_size, max(0, len(right_bay) - 1))
            for left_block_size in range(1, left_block_cap + 1):
                for right_block_size in range(1, right_block_cap + 1):
                    ...
                    if len(left_prefix) == 0 or len(right_suffix) == 0:
                        continue
                    merged_sequence = left_prefix + right_prefix + left_suffix + right_suffix
                    if merged_sequence == left_bay + right_bay:
                        continue
                    natural_split = int(len(left_prefix) + len(right_prefix))
                    if natural_split <= 0 or natural_split >= len(merged_sequence):
                        continue
                    best_cut_score = min(
                        self._evaluate_two_stage_split_cut_flow(sym_flow, merged_sequence, split_idx)
                        for split_idx in range(1, len(merged_sequence))
                    ...
                    exchange_candidates.append(
                        {
                            ...
                            "current_split": natural_split,
                            "variant_kind": "exchange",
                            "left_block_size": int(left_block_size),
                            "right_block_size": int(right_block_size),
                            "best_cut_score": float(best_cut_score),
                        }
                    )
            exchange_candidates.sort(
                key=lambda item: (
                    float(item["best_cut_score"]),
                    abs(int(item["current_split"]) - int(len(left_bay))),
                    int(item["left_block_size"]) + int(item["right_block_size"]),
                )
            )
            exchange_variant_cap = int(max(0, getattr(self, "two_stage_action14_reorder_variants_per_pair", 1)))
            variant_entries.extend(exchange_candidates[:exchange_variant_cap])

            for variant_entry in variant_entries:
                merged_sequence = [int(facility) for facility in variant_entry["merged_sequence"]]
                current_split = int(variant_entry["current_split"])
                prefix_areas = []
                cumulative_area = 0.0
                for facility in merged_sequence:
                    facility_idx = int(facility) - 1
                ...
                cut_scores.sort(key=lambda item: item[0])
                top_cut_splits = {int(split_idx) for _score, split_idx in cut_scores[:2]}
                ...
                for split_idx in range(1, len(merged_sequence)):
                    ...
                    if best_area_split is not None and int(split_idx) == int(best_area_split):
                        template_name = "balanced_cut"
                    elif int(split_idx) in top_cut_splits:
                        template_name = "cut_min"
                    elif abs(int(split_idx) - int(current_split)) <= 1:
                        template_name = "boundary_shift"
                    else:
                        template_name = "repartition"
                    if str(variant_entry.get("variant_kind", "base")) == "exchange":
                        template_name = f"exchange_{template_name}"
                    candidate_meta = {
                        "pair_idx": int(pair_idx),
                        "split_idx": int(split_idx),
                        "current_split": int(current_split),
                        "merged_sequence": list(merged_sequence),
                        "variant_kind": str(variant_entry.get("variant_kind", "base")),
                        "left_block_size": int(variant_entry.get("left_block_size", 0)),
                        "right_block_size": int(variant_entry.get("right_block_size", 0)),
                        "template": f"cross_pair/{template_name}",
                    }
                    self._append_two_stage_structure_candidate(
                        proposals,
    ...
    def _select_two_stage_survivors(self, scored_candidates, eval_budget, action_idx=None, context=None):
        if not scored_candidates:
            return []
        ...
        if len(ordered) <= eval_budget:
            return ordered
        if (
            self.two_stage_learned_evaluator_enabled
            and self.two_stage_learned_survivor_diversity_enabled
            and context is not None
        ):
            def _entry_key(candidate_entry):
                return (
                    int(candidate_entry.get("proposal_idx", -1)),
                    tuple(candidate_entry.get("permutation", [])),
                    tuple(candidate_entry.get("bay", [])),
                )

            def _append_survivor(candidate_item, survivors, seen_entries):
                _, candidate_entry = candidate_item
                candidate_key = _entry_key(candidate_entry)
                if candidate_key in seen_entries:
                    return False
                seen_entries.add(candidate_key)
                survivors.append(candidate_item)
                return True
            ...
            seen_entries = set()
            signature_cache = {
                _entry_key(entry): self._build_two_stage_candidate_signature(context, entry)
                for _score, entry in ordered
            }
            if ordered:
                _append_survivor(ordered[0], survivors, seen_entries)
            score_values = [float(score) for score, _entry in ordered]
            score_min = min(score_values) if score_values else 0.0
            score_max = max(score_values) if score_values else score_min
            ...
            while len(survivors) < survivor_count:
                ...
                for candidate_item in ordered:
                    candidate_score, candidate_entry = candidate_item
                    candidate_key = _entry_key(candidate_entry)
                    if candidate_key in seen_entries:
                        continue
                    normalized_quality = (float(candidate_score) - float(score_min)) / float(score_span)
                    redundancy_penalty = 0.0
                    candidate_signature = signature_cache.get(candidate_key)
                    for _selected_score, selected_entry in survivors:
                        selected_key = _entry_key(selected_entry)
                        selected_signature = signature_cache.get(selected_key)
                        redundancy_penalty = max(
                            redundancy_penalty,
                            self._two_stage_candidate_similarity(candidate_signature, selected_signature),
                        )
                    marginal_score = float(normalized_quality) - (
                        float(self.two_stage_learned_survivor_diversity_weight) * float(redundancy_penalty)
                    )
                    if (
                        best_item is None
                        or float(marginal_score) > float(best_marginal_score)
                        or (
                            math.isclose(float(marginal_score), float(best_marginal_score), rel_tol=1e-9, abs_tol=1e-9)
                            and float(candidate_score) > float(best_item[0])
                        )
                    ):
                        best_item = candidate_item
                        best_marginal_score = float(marginal_score)
                if best_item is None:
                    break
                _append_survivor(best_item, survivors, seen_entries)

            if self.two_stage_random_survivor and len(survivors) < eval_budget:
                remaining = [item for item in ordered if _entry_key(item[1]) not in seen_entries]
                if remaining:
                    random_idx = int(np.random.randint(0, len(remaining)))
                    _append_survivor(remaining[random_idx], survivors, seen_entries)
    ...
    def _generate_two_stage_heavy_candidate(
        ...
        if self.two_stage_action14_exact_eval_enabled and int(action_idx) == 14:
            proposals = self._propose_two_stage_adjacent_bay_repartition_full_scan(context)
            ...
            exact_candidate = self._generate_two_stage_action14_exact_candidate(
                solution,
                context,
                scored_candidates,
                telemetry,
            ...
            self.logger.warning(
                "动作14精确增量评估不可用，回退到原始two-stage路径 | "
                f"group_id: {group_id} | proposals: {len(proposals)}"
            )
        ...
        if self.two_stage_learned_evaluator_enabled:
            learned_score_map = self.dqn_two_stage_evaluator.score_candidates_batch(
                self,
                context,
                proposals,
                action_idx,
        ...
        for proposal_idx, (permutation, bay, meta) in enumerate(proposals):
            ...
            learned_score = None
            learned_record = None
            if self.two_stage_learned_evaluator_enabled and int(proposal_idx) in learned_score_map:
                learned_score, learned_record = learned_score_map[int(proposal_idx)]
            if learned_score is not None and np.isfinite(learned_score):
                stage1_score = float(learned_score)
                if self.dqn_two_stage_evaluator.collect_data:
                ...
                if not np.isfinite(proxy_score):
                    continue
                stage1_score = float(proxy_score)
                if self.two_stage_learned_evaluator_enabled:
            ...
            scored_candidates.append(
                {
                    ...
                    "proxy_score": (
                        float(proxy_score)
                        if proxy_score is not None and np.isfinite(proxy_score)
                        else None
                    ),
                    "learned_score": (
                        float(learned_score)
        ...
        if not scored_candidates:
            if self.two_stage_learned_evaluator_enabled:
                self.logger.warning(
                    "Two-stage learned evaluator 当前 group 无有效打分，整体回退到 proxy | "
                    f"group_id: {group_id} | action: {int(action_idx)} | proposals: {len(proposals)}"
                )
            telemetry["fallbacks"] += 1
            return self._generate_candidate_by_action_fallback(solution, action_idx, phase=fallback_phase)
        if self.two_stage_learned_evaluator_enabled and learned_fallback_count > 0:
            self.logger.warning(
                "Two-stage learned evaluator 部分 proposal 打分失败，已按需回退到 proxy | "
                f"group_id: {group_id} | action: {int(action_idx)} | "
                f"fallbacks: {int(learned_fallback_count)}/{int(len(proposals))}"
            )
                ...
                dataset_records.append(
                    {
                        ...
                        "template": str(learned_record.get("template", "default")),
                        "proxy_score": (
                            _safe_record_float(candidate_entry.get("proxy_score"))
                            if candidate_entry.get("proxy_score") is not None
                            else None
                        ),
                        "learned_score": (
    ...
    def _greedy_local_search(
        ...
        enable_guided=False,
        guided_topk=None,
        guided_max_iters=None,
        observe_archive=True,
    ):
        """
