...
from src.algorithms.RL.QLearning2 import QLearningAgent
    ...
    def run(self):
        ...
        while g < self.G:  # 外层循环：g < G（图片中算法外层循环条件）
            ...
            while t < self.t_max:  # 内层循环：t < t_max（图片中算法内层循环条件）
                ...
                op = agent.select_action(0, is_training=True)
                s_prime = self._generate_new_solution(self.s, op)
...
if __name__ == "__main__":
    ...
    exp_algorithm = "ELP_QLearning2"  # 算法名称改为ELP
    exp_remark = "k=20*T/T_initial,bin_width=5,t_max = 300,G=2000,getfitness2，修改了bay_flip操作，g_best=4730，惩罚指数：1至5，操作1-6,有εQLearning"
    exp_number = 30
    is_exp = True
