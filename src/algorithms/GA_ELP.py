...
class ELP:
    def __init__(self, env, T, Q_matrix, G=100, t_max=50, k=0.1,
    ...
    def run(self):
        ...
        if s.fbs_model!=pop_best_solution :
            print("pop_best_solution与self.env不相同")
        else:
            print("pop_best_solution与self.env相同")
        # print("reset后环境中的fbs_model：")
        # print(f"env.fbs_model: {s.fbs_model}")
        # print(f"是否与最优解一致: {s.fbs_model == pop_best_solution}")  # 需FBSModel实现__eq__方法
        # # 或对比内存地址
        # print(f"内存地址是否相同: {id(s.fbs_model) == id(pop_best_solution)}")
        self.current_energy = pop_best_fitness # 在个实验里能量就等于适应度
        self.gBest = copy.deepcopy(s)
        ...
        while g < self.G:  # 外层循环：g < G（图片中算法外层循环条件）
            ...
            while t < self.t_max:  # 内层循环：t < t_max（图片中算法内层循环条件）
                ...
                s_prime = self._generate_new_solution(s)
                ...
                if delta_E_prime < 0 or r < math.exp(-delta_E_prime / self.T):
                    # 接受新解：更新当前解和当前能量
...
if __name__ == "__main__":
    ...
    exp_algorithm = "ELP_with_PopOpt"  # 算法名称改为ELP
    exp_remark = "加上了种群优化算法"
    exp_number = 30
    is_exp = True
