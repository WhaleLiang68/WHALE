...
class DoubleQLearningAgent:
    def __init__(
        ...
        self.c_bonus = float(c_bonus)
        self.Q1 = np.zeros((s_dim, a_dim), dtype=float)
        self.Q2 = np.zeros((s_dim, a_dim), dtype=float)
        self.N = np.zeros((s_dim, a_dim), dtype=np.int32)
        self.P = np.zeros((s_dim, a_dim), dtype=float)

    def set_action_priors(self, prior_values):
        prior_values = np.asarray(prior_values, dtype=float).reshape(1, -1)
        if prior_values.shape[1] != self.a_dim:
            raise ValueError(f"Expected {self.a_dim} priors, got {prior_values.shape[1]}")
        self.P[:] = prior_values

    def _exploration_params(self, no_improve_steps):
        if no_improve_steps < 20:
            return 0.05, 0.10
        if no_improve_steps < 60:
            return 0.12, 0.20
        return 0.22, 0.40

    def _allowed_actions_array(self, allowed_actions):
        if allowed_actions is None or len(allowed_actions) == 0:
            return np.arange(self.a_dim, dtype=int)
        return np.asarray(allowed_actions, dtype=int).reshape(-1)

    def _score_actions(self, s, allowed_actions):
        q_bar = 0.5 * (self.Q1[s, allowed_actions] + self.Q2[s, allowed_actions])
        prior_bias = self.beta * self.P[s, allowed_actions]
        exploration_bonus = self.c_bonus / np.sqrt(self.N[s, allowed_actions] + 1.0)
        return q_bar + prior_bias + exploration_bonus
    ...
    def select_action(self, s, no_improve_steps=0, deterministic=False, allowed_actions=None):
        allowed_actions = self._allowed_actions_array(allowed_actions)
        if allowed_actions.size == 1:
            return int(allowed_actions[0])

        scores = self._score_actions(s, allowed_actions)
        if deterministic:
            max_score = np.max(scores)
            best_actions = allowed_actions[np.isclose(scores, max_score)]
            return int(np.random.choice(best_actions))

        eps_s, tau_s = self._exploration_params(no_improve_steps)
        logits = scores / max(float(tau_s), 1e-8)
        soft_policy = self._softmax(logits)
        uniform_policy = np.full(allowed_actions.size, 1.0 / allowed_actions.size, dtype=float)
        policy = (1.0 - eps_s) * soft_policy + eps_s * uniform_policy
        policy = policy / np.sum(policy)
        return int(np.random.choice(allowed_actions, p=policy))

    def update_Q(self, s, a, reward, s_next, done=False, allowed_next_actions=None):
        update_q1 = np.random.rand() < 0.5
        target = float(reward)
        if not done:
            next_actions = self._allowed_actions_array(allowed_next_actions)
            if next_actions.size > 0:
                q_bar_next = 0.5 * (self.Q1[s_next, next_actions] + self.Q2[s_next, next_actions])
                max_q = np.max(q_bar_next)
                best_next_actions = next_actions[np.isclose(q_bar_next, max_q)]
                a_star = int(np.random.choice(best_next_actions))
                if update_q1:
                    target += self.gamma * self.Q2[s_next, a_star]
                else:
                    target += self.gamma * self.Q1[s_next, a_star]
...
class ELP:
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0):
        ...
        self.base_temperature_seed = float(T)
        self.G = int(G)
        self.t_max = int(t_max)
        self.k_hist = float(k)
        self.k_penalty = 1
        self.cooling_per_step = 0.999
        self.temperature_floor_samples = 100
        self.temperature_floor_target_accept = 0.03
        self.temperature_floor_quantile = 12
        self.temperature_floor_cap_ratio = 0.18
        self.initial_temperature_samples = 100
        self.initial_temperature_quantile = 90
        self.T_min = max(self.T_initial * 0.05, 1.0)
        self.enable_acceptance_thermostat = True
        self.acceptance_thermostat_window = 1200
        self.acceptance_thermostat_gain = 0.15
        self.acceptance_thermostat_min_factor = 0.995
        self.acceptance_thermostat_max_factor = 1.005
        self._thermostat_worse_total = 0
        self._thermostat_worse_accept = 0
        self.action_recipes = {
            0: [0],
            2: [2],
            3: [3],
        ...
        self.temperature_floor_action_ids = [10, 2]
        ...
        self.elite_actions = [10, 9, 2, 11, 0]
    ...
    def _sample_initial_temperature_from_neighborhood(self, base_solution, sample_size=None, quantile=None):
        sample_size = int(sample_size or self.initial_temperature_samples)
        quantile = float(quantile or self.initial_temperature_quantile)
        fallback = max(float(self.base_temperature_seed), 1.0)

        if (
            base_solution is None
            or not np.isfinite(getattr(base_solution, "fitness", np.inf))
            or sample_size <= 0
        ):
            return fallback

        base_cost = float(base_solution.fitness)
        deltas = []

        # Stage 1 neighbors: adjacent bay swap
        bay_structure = permutationToArray(
            base_solution.fbs_model.permutation,
            base_solution.fbs_model.bay,
        )
        for bay_idx in range(max(0, len(bay_structure) - 1)):
            candidate_structure = bay_structure[:]
            candidate_structure[bay_idx], candidate_structure[bay_idx + 1] = (
                candidate_structure[bay_idx + 1],
                candidate_structure[bay_idx],
            )
            new_perm, new_bay = arrayToPermutation(candidate_structure)
            candidate = copy.deepcopy(base_solution)
            candidate.fbs_model.permutation = new_perm
            candidate.fbs_model.bay = new_bay
            self._evaluate_solution(candidate)
            if np.isfinite(candidate.fitness):
                delta = abs(float(candidate.fitness) - base_cost)
                if delta > 0:
                    deltas.append(delta)
            if len(deltas) >= sample_size:
                break

        # Stage 2 neighbors: adjacent facility swap in each bay
        if len(deltas) < sample_size:
            bay_structure = permutationToArray(
                base_solution.fbs_model.permutation,
                base_solution.fbs_model.bay,
            ...
            for bay_idx, bay in enumerate(bay_structure):
                ...
                if len(bay) < 2:
                    continue
                for j in range(len(bay) - 1):
                    candidate_structure = [list(b) for b in bay_structure]
                    candidate_structure[bay_idx][j], candidate_structure[bay_idx][j + 1] = (
                        candidate_structure[bay_idx][j + 1],
                        candidate_structure[bay_idx][j],
                    )
                    new_perm, new_bay = arrayToPermutation(
                        [np.array(b) for b in candidate_structure]
                    )
                    candidate = copy.deepcopy(base_solution)
                    candidate.fbs_model.permutation = new_perm
                    candidate.fbs_model.bay = new_bay
        ...
        quantile = min(max(quantile, 0.0), 100.0)
        sampled_t0 = float(np.percentile(deltas, quantile))
        if not np.isfinite(sampled_t0) or sampled_t0 <= 0:
            return fallback
        return sampled_t0
    ...
    def _activate_main_search_from_feasible(self):
        ...
        self._evaluate_solution(self.s)
        self.current_energy = self.s.fitness
        sampled_t0 = self._sample_initial_temperature_from_neighborhood(self.s)
        self.T_initial = max(float(sampled_t0), 1.0)
        self.T = self.T_initial
        logger.info(
            f"Adaptive initial temperature sampled | T0: {self.T_initial:.6f} | samples: {self.initial_temperature_samples} | quantile: {self.initial_temperature_quantile}%"
        )
        self.no_improve_steps = 0
        self.episodes_without_improvement = 0
        self.last_diversify_step = -max(1, self.t_max)
    ...
    def _prepare_episode_start(self, episode_idx):
        ...
        self._evaluate_solution(candidate)
        if (
            np.isfinite(candidate.fitness)
            and np.isfinite(self.s.fitness)
            and candidate.fitness < self.s.fitness
        ):
            self.s = candidate
            self.current_energy = self.s.fitness
    ...
    def _attempt_diversification(self, global_step):
        ...
        if accept:
            ...
            self.s = candidate
            self.current_energy = self.s.fitness
            improved = self._observe_feasible_state(self.s)
    ...
    def _thermostat_target_accept(self):
        if self.no_improve_steps < 20:
            return 0.22
        if self.no_improve_steps < 60:
            return 0.18
        return 0.14

    def _update_acceptance_thermostat(self):
        if not self.enable_acceptance_thermostat:
            return
        if self._thermostat_worse_total < self.acceptance_thermostat_window:
            return
        observed = self._thermostat_worse_accept / max(float(self._thermostat_worse_total), 1.0)
        target = self._thermostat_target_accept()
        error = target - observed
        factor = math.exp(self.acceptance_thermostat_gain * error)
        factor = min(max(factor, self.acceptance_thermostat_min_factor), self.acceptance_thermostat_max_factor)
        self.T = max(self.T_min, float(self.T) * float(factor))
        self._thermostat_worse_total = 0
        self._thermostat_worse_accept = 0
    ...
    def _run_impl(self):
        ...
        prior_values = np.array([
            prior.get(action_idx, 0.03) for action_idx in self.valid_actions
        ], dtype=float)
        agent.set_action_priors(prior_values)
        ...
        for episode in range(self.G):
            ...
            for _ in range(self.t_max):
                ...
                accept, prob, _, _ = self._accept_candidate(self.s.fitness, candidate.fitness)
                self.prob_history.append(prob)
                is_worsening_proposal = bool(
                    np.isfinite(previous_cost)
                    and np.isfinite(candidate.fitness)
                    and candidate.fitness >= previous_cost
                )
                if is_worsening_proposal:
                    self._thermostat_worse_total += 1
                improved = False
                action_hit_global = False

                if accept:
                    self.s = candidate
                    self.current_energy = self.s.fitness
                    action_hit_global = self._observe_feasible_state(self.s)
                    if action_hit_global:
                    ...
                    improved = bool(action_hit_global)
                    trigger_local_search = bool(action_hit_global)
                    self._update_histogram(self.s.fitness)
                    if improved:
                        self.no_improve_steps = 0
                        fast_time = datetime.datetime.now()
                    ...
                    if trigger_local_search:
                        ...
                        self.current_energy = self.s.fitness
                        post_local_global = bool(
                            np.isfinite(self.best_feasible_cost)
                            and self.best_feasible_cost < previous_best_feasible
                        )
                        if post_local_global and not action_hit_global:
                            self._record_action_global_best(real_action_idx, phase="main")
                        improved = post_local_global

                else:
                    self._update_histogram(self.s.fitness)
                    self.no_improve_steps += 1

                if accept and is_worsening_proposal:
                    self._thermostat_worse_accept += 1
                ...
                self._update_acceptance_thermostat()
...
if __name__ == "__main__":
    ...
    exp_remark = "两阶段仅全局最优时触发，重启/扰动候选非合法也接受+ELP+full-Q-learning,移除了 bay_flip、facility_insert (5) 和 facility_shuffle (7)+先验质量设初始值,改 archive 更新时机 + 换 Double Q / softmax”"
    exp_number = 50
    is_exp = True
