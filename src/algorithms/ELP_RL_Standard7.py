...
class ELP:
    def __init__(self, env, gbest, T, G=100, t_max=50, k=2.0):
        ...
        self.temperature_floor_action_ids = [9, 2, 0, 6]
        self.valid_actions = [0, 2, 6, 9, 1]
        self.action_labels = {
            0: "facility_swap",
            1: "bay_flip",
            2: "bay_swap",
        ...
        self.light_restart_recipe = [5, 0, 3]
        self.diversify_recipe = [6, 7, 3]
        self.bay_flip_action_id = 1
        self.bay_flip_max_selections_per_episode = 10
        self.bay_flip_selected_this_episode = 0
        self.bin_width = 50.0
        ...
        self.telemetry_action_ids = sorted(set(self.valid_actions) | set(self.elite_actions))
        self.action_telemetry = {
            ...
            for action_idx in self.telemetry_action_ids
        }
    ...
    def _get_allowed_action_indices(self, solution):
        ...
        allowed = [
            ...
            and (
                action_idx != self.bay_flip_action_id
                or self.bay_flip_selected_this_episode < self.bay_flip_max_selections_per_episode
            )
        ]
    ...
    def _greedy_local_search(self, solution):
        """
        闁荤姳鑳堕崕銈夊几閸愵亞绠?.3闂佺厧鎼崑濠勬閻楀牏鈻旈柕鍫濐槹閳诲绻涢崼婊呯シ婵＄偠濮ょ换婵嬵敂閸曨厽鎷遍梻渚囧枔閸斿秹骞冨畷鍥潟妞ゎ偒鍏橀崑?
        闂侀潻璐熼崝宥夋儊閳ユ枼鏋庨柍鈺佸暙婢跺秹鏌涘▎鎰垫畷闁哄苯锕﹂幉鎾晲閸涱喖鈧敻鎮圭€ｎ亜鏆熼柡浣靛€濋弫宥囦沪缁涘娈搁悷婊呭閹稿憡鏅堕悩鍨枂闁挎繂鎳嶇划鐢告倵闂堟稒顥㈡繛澶涚畱椤曪綁鍩€椤掑嫭鐓傞柛銉ｅ劗閸?

        Stage 1闂佹寧绋掑畝鎼佸Υ鎼淬垺鍎熼煫鍥ㄧ◥閹查箖鏌涘Ο鐑橆棡濠⒀冪Ч瀵灚寰勭仦鍓с偛闂?bay 闁诲海鏁稿▍銏㈡濠靛洨顩查柕鍫濇鎼村﹪鏌涘顒佸攭閻庢哎鍨藉銊╂偡閻楀牊绶伴梺鍛婂笚閻燂妇鏁幘顔肩煑婵°倕顑囩粈?
                 闂備焦褰冪粔鎾囬弻銉﹀剮闁哄鍨甸悡鍌氣槈閹绢垰浜鹃柡澶屽剱閸犳螞閵堝缁╅柣鐔哄濠㈡牕鈽夐幘璺哄妺妞ゆ帗鐩俊?
        Stage 2闂佹寧绋掗懝楣冾敋椤斿す鎺曠疀鎼淬劌娈?bay闂佹寧绋戦惌鍌炲Υ鎼淬垺鍎熼煫鍥ㄧ◥閹查箖鏌涘Ο鐑橆棞闁告瑩绠栧畷姗€宕ㄩ弶鎴濆Г闂佺儵鏅滈幃鍌炲磻閿旂偓濯奸柣鎰靛墯閻撱垽鎮楅悽鍨仧缂?
                 婵炲瓨鍤庨崐鏍х暦閺屻儱瑙﹂幖娣€曢鐔兼煛閳ь剟鎮烽悧鍫熺钒闂佸憡甯楅悷锔炬暜閹绢喖鐭楁俊銈咁儑缁€澶愭⒑閹绘帞孝妞わ附鐓￠幆鍕级閹存繄鍘掓繛鎴炴尨閸嬫捇寮堕悜鍡楁灍婵☆偁鍊濆銊╂偡閻楀牊绶版繛鎴炴尵閸庛倝顢楀☉銏犖?
        """
        improved = True
        while improved:
            improved = False

            # 闂佸啿鍘滈崑鎾绘煃閸忓浜?Stage 1闂佹寧绋掑銊ッ规径鎰劷?bay 婵炲瓨鍤庨崐鏍х暦?闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜?
            ...
            for i in range(n_bays - 1):
                # 闂佸搫顑呯€氫即鍩€椤掑倸小婵﹫绠撻獮鎴﹀閻樺啿鈧敻鏌?bay 缂傚倷鐒﹂幐濠氭倵?
                ...
                # 闂佹眹鍨婚崰鎰板垂濮樿泛纾规繛鍡樻尨閸嬫挻寰勯獮顔藉浮閻涱噣骞嗛幍顔筋潠婵?
                candidate = copy.deepcopy(solution)
                ...
                # 闁荤姵鍔︽禍婊堝煘閸℃稑绠抽柕澶堝劚缂嶁偓闂佹寧绋掗懝鎹愩亹瑜旈獮鎺楀Ψ閵夈儳绉梺娲讳簽閸犲酣鎮块崟顖涘殑濞寸姴顑呭▍銈呪槈閹炬剚鍎旀俊?
                if (
                        candidate.current_is_feasible
                        and np.isfinite(candidate.fitness)
                    ...
                    solution = candidate
                    # 闂佸搫娲ら悺銊╁蓟?bay_structure 婵炲濮伴崕鎵矚閸洖瑙﹂幖杈剧悼閺佺偓鎱ㄥ┑鍕姦妞ゅ骸鍟村畷鐑樻姜閹殿喛澹橀梺鍝勫€规刊鍊燁暰
                    bay_structure = permutationToArray(
                        solution.fbs_model.permutation,
                        solution.fbs_model.bay,
                    )
            ...
            # 闂佸啿鍘滈崑鎾绘煃閸忓浜?Stage 2闂佹寧绋掔喊宥夋儊閳╁啰鈻?bay 闂佸憡鍔曢幊鎰规径鎰劷闁哄嫬娴氶崯搴ㄦ煛閸屾稑鐏婃慨姗堢畵楠?闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕闂佸啿鍘滈崑鎾绘煃閸忓浜鹃梺鍐插帨閸嬫捇鏌嶉崗澶婁壕
            bay_structure = permutationToArray(
                solution.fbs_model.permutation,
                solution.fbs_model.bay,
            )
            ...
            for bay_idx, bay in enumerate(bay_structure):
                ...
                for j in range(n_fac - 1):
                    # 婵炲瓨鍤庨崐鏍х暦?bay 闂佸憡鍔曢幊鎰邦敄?j 闂?j+1 婵炴垶鎼╂禍锝夘敊閺囥垹妫?
                    ...
                    if (
                        ...
                        solution = candidate
                        # 闂佸搫娲ら悺銊╁蓟婵犲懌浜归柟鎯у暱椤?bay_structure 缂傚倷缍€閸涱垱鏆伴梺鍛婅壘濞寸兘鎮界紒妯碱洸闁靛牆妫楁惔?
                        bay_structure = permutationToArray(
                            solution.fbs_model.permutation,
                            solution.fbs_model.bay,
                        )
    ...
    def _run_impl(self):
        ...
        for episode in range(self.G):
            ...
            for _ in range(self.t_max):
                ...
                if real_action_idx == self.bay_flip_action_id:
                    self.bay_flip_selected_this_episode += 1
                previous_cost = self.s.fitness
                previous_d_inf = self.s.current_d_inf
                ...
                if accept:
                    ...
                    self.current_energy = self.s.fitness
                    # Run the two-stage local search after every accepted move.
...
if __name__ == "__main__":
    ...
    exp_remark = "ELP+full-Q-learning+histogram band+two-stage full run"
    exp_number = 50
    is_exp = True
