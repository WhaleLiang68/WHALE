...
class Q_LearningELP:
    ...
    def greedy_two_stage_search(self, s):
        ...
        def _greedy_optimize(state, action, max_attempts):
            """贪婪优化子函数（提取重复逻辑）"""
            improved = True
            ...
            while improved and max_attempts > 0:
                ...
                # 执行指定动作
                _, _, _, info = current_state.step2(action=action)

                # 检查是否改进（适应度降低表示更优）
                if info["current_fitness"] < original_state.fitness:
                    improved = True
                else:
                    current_state = original_state  # 回退到原状态

                max_attempts -= 1
            return current_state

        # 阶段1：区带内局部优化（bay_shuffle，动作5）
        s = _greedy_optimize(s, action=5, max_attempts=50)

        # 阶段2：全局优化（facility_shuffle，动作6）
        s = _greedy_optimize(s, action=6, max_attempts=20)
    ...
    def run(self):
        ...
        self.current_energy = self.env.fitness # 在个实验里能量就等于适应度
        self.gBest = s
...
if __name__ == "__main__":
    ...
    exp_algorithm = "LELP"  # 算法名称改为ELP
    exp_remark = "加上qlearning"
    exp_number = 30
    is_exp = True
