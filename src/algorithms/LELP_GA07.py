...
class ELP:
    ...
    def run(self):
        ...
        agent = QLearningAgent(s_dim=1, a_dim=4, epsilon=0.8, initial_fitness=self.current_energy)
        ...
        while g < self.G:  # 外层循环：g < G（图片中算法外层循环条件）
            ...
            while t < self.t_max:  # 内层循环：t < t_max（图片中算法内层循环条件）
                ...
                if accept:
                    ...
                    self.s = s_prime
                    self.current_energy = E_s_prime
                    # 如果使用的是Q-Learning选择的动作（op < 5），更新Q值
                    if op < 7:
...
if __name__ == "__main__":
    ...
    exp_algorithm = "GA_ELP_QLearning"  # 算法名称改为ELP
    exp_remark = "k=15*T/T_initial,bin_width=5,t_max = 300,G=2000,getfitness2，修改了bay_flip操作，g_best=5396.6，惩罚指数：1至5，操作1-4，贪婪0-3"
    exp_number = 30
    is_exp = True
