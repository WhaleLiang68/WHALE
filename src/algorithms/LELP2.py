...
class LELP:
    def __init__(self, env, gbest, T, Q_matrix, G=100, t_max=50, k=0.1,num_operators=5):
        """
    ...
    def run(self):
        ...
        # 初始化解
        agent = QLearningAgent(s_dim=1, a_dim=3,epsilon=0.8,initial_fitness=self.current_energy)
        T_matrix = []
        while g < self.G:  # 外层循环：g < G（图片中算法外层循环条件）
            ...
            while t < self.t_max:  # 内层循环：t < t_max（图片中算法内层循环条件）
                ...
                op = agent.select_action(env, s=0)
                ...
                agent.update_Q(s=0, a=op, f_ns=E_s_prime)
            ...
            # 将当前Q矩阵添加到列表中
            Q_matrix.append({
                "epoch": g,  # 记录循环次数（便于后续分析）
                "q_matrix": agent.Q
            })

            T_matrix.append({
                "epoch": g,  # 记录循环次数（便于后续分析）
                "t_matrix": agent.times
            })
            g += 1
            # agent.save()
        ...
        np.save(filepath1, agent.Q)
        filename2 = f"times_table.npy"
        filepath2 = os.path.join(save_dir, filename2)
        np.save(filepath2, agent.times)
        end_time = datetime.datetime.now()
        print("Q矩阵：",agent.Q)
        print("t矩阵：",agent.times)
...
if __name__ == "__main__":
    ...
    exp_algorithm = "LELP2"  # 算法名称改为ELP
    exp_remark = "qlearning的探索率为0.8，最小衰减率为0.05,可选择操作只有3个"
    exp_number = 30
    is_exp = True
    ...
    if is_exp:
        ...
        for i in range(exp_number):
            ...
            try:
                ...
                # 3. 运行ELP算法
                total_iter, best_sol, best_energy, start, end, fast,Q_matrix = elp_solver.run()
                best_fitness=best_sol.fitness

                # 4. 输出结果
                logger.info(f"第{i + 1}次实验完成 | 最优能量: {best_energy}")
                print(f"Best Solution: {best_sol.fbs_model.array_2d}, Best Energy: {best_energy},Best Fitness: {best_fitness}")
                ...
                ExperimentsUtil.save_experiment_result(
                    ...
                    exp_fitness=best_fitness,  # 能量对应原fitness，字段名保持兼容
