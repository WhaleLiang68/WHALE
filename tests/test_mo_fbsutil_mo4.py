...
class TestMOFBSUtil(unittest.TestCase):
    ...
    def test_archive_igd_is_zero_for_exact_reference_match(self):
        candidates = [
            SimpleNamespace(mo_objectives_min=np.array([0.2, 0.4, 0.6, 0.8], dtype=float)),
            SimpleNamespace(mo_objectives_min=np.array([0.8, 0.6, 0.4, 0.2], dtype=float)),
        ]
        reference_vectors = [
            np.array([0.2, 0.4, 0.6, 0.8], dtype=float),
            np.array([0.8, 0.6, 0.4, 0.2], dtype=float),
        ]

        igd = MO_FBSUtil.archive_igd(
            candidates,
    ...
    def test_update_archive_uses_reference_front_to_keep_gap_filling_point(self):
        archive = [
            SimpleNamespace(
                current_is_feasible=True,
                current_d_inf=0,
            ...
            SimpleNamespace(
                ...
                current_d_inf=0,
                constraint_violation=0.0,
                mo_objectives_min=np.array([0.9, 0.1, 0.1, 0.1], dtype=float),
            ),
        ]
        gap_filling_candidate = SimpleNamespace(
            current_is_feasible=True,
            current_d_inf=0,
            constraint_violation=0.0,
            mo_objectives_min=np.array([0.35, 0.35, 0.35, 0.35], dtype=float),
        )

        updated_archive, inserted, _ = MO_FBSUtil.update_pareto_archive(
            archive,
            gap_filling_candidate,
            max_size=3,
            require_candidate_retained=True,
            reference_vectors=[np.array([0.35, 0.35, 0.35, 0.35], dtype=float)],
        )

        self.assertTrue(inserted)
        self.assertEqual(len(updated_archive), 3)
        self.assertTrue(
            any(
                np.allclose(item.mo_objectives_min, gap_filling_candidate.mo_objectives_min)
                for item in updated_archive
            )
        )
        self.assertFalse(
            any(
                np.allclose(item.mo_objectives_min, np.array([0.9, 0.1, 0.1, 0.1], dtype=float))
                for item in updated_archive
            )
        )
