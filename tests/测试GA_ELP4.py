...
class ELP:
    def __init__(self, env, gbest, T, Q_matrix, G=100, t_max=50, k=0.1):
        ...
        [修改] 增加了 debug_run_log.csv 日志记录器
        """
        self.env = env
        self.gbest = copy.deepcopy(gbest)  # 初始全局最优解
        self.T = T  # 初始温度
        self.Q_matrix = copy.deepcopy(Q_matrix)  # Q值矩阵
        self.G = G  # 外层循环最大迭代次数
        self.t_max = t_max  # 内层循环最大步数
        self.k = k  # 能量计算系数k
        self.action_5_prob = 0.1
        self.action_5_prob_min = 0.001  # 动作5的最小概率阈值
        self.action_5_decay = 0.85  # 动作5概率衰减系数（从0.05改为0.95，衰减更慢）

        # 初始化当前解和能量
        self.s = copy.deepcopy(gbest)
        self.current_energy = self._calculate_energy(self.s)
        self.best_energy = self.current_energy  # 记录最优能量
        self.energy_histogram = {}

        # --- 新增调试日志 ---
        self.debug_log_file = "debug_run_log.csv"
        try:
            with open(self.debug_log_file, 'w', encoding='utf-8') as f:
                f.write(
                    "g,t,action,E_before,E_candidate,delta_E_prime,T,acceptance_prob,r,accepted,E_current,E_best,candidate_perm,current_perm\n")
            logger.info(f"详细调试日志将写入: {self.debug_log_file}")
        except Exception as e:
            logger.error(f"无法创建调试日志文件: {e}")
    ...
    def _generate_new_solution(self, s):
        ...
        op = np.random.randint(0, 5)  # (原代码是 0, 4，保持不变)
    ...
    def _check_aspect_ratio_constraint(self, solution):
        ...
        try:
            ...
            if np.any(fac_aspect_ratio > self.env.fac_limit_aspect):
                logger.debug(
                    f"宽高比检查失败: 最大宽高比 {np.max(fac_aspect_ratio):.2f} > 限制 {self.env.fac_limit_aspect}")
                return False

            logger.debug(
                f"宽高比检查通过: 最大宽高比 {np.max(fac_aspect_ratio):.2f} <= 限制 {self.env.fac_limit_aspect}")
            return True
    ...
    def run(self):
        ...
        while g < self.G:  # 外层循环
            ...
            while t < self.t_max:  # 内层循环

                # 1. [修改] 产生新解 s' 并获取所用的动作 op
                s_prime, op = self._generate_new_solution(self.s)
                ...
                if delta_E_prime >= 0:
                    try:
                        acceptance_prob = math.exp(-delta_E_prime / (self.T * k_boltzmann))
                    except OverflowError:
                        acceptance_prob = 0.0
                # --- 调试变量结束 ---

                if delta_E_prime < 0 or r < acceptance_prob:
                    # 接受新解
                    accepted = True
                ...
                try:
                    # 尝试获取布局信息
                    candidate_perm = s_prime.fbs_model.permutation if hasattr(s_prime, 'fbs_model') else 'N/A'
                    current_perm = self.s.fbs_model.permutation if hasattr(self.s, 'fbs_model') else 'N/A'

                    with open(self.debug_log_file, 'a', encoding='utf-8') as f:
                        log_line = (
                            f"{g},{t},{op},{E_s:.2f},{E_s_prime:.2f},{delta_E_prime:.2f},"
                            f"{self.T:.2f},{acceptance_prob:.4f},{r:.4f},{accepted},"
                            f"{self.current_energy:.2f},{self.best_energy:.2f},"
                            f"\"{candidate_perm}\",\"{current_perm}\"\n"
                        )
                        f.write(log_line)
                except Exception as e:
                    logger.error(f"G={g}, t={t}: 写入调试日志失败: {e}")
                # --- 调试日志结束 ---
...
if __name__ == "__main__":
    ...
    exp_algorithm = "ELP_with_Pop_action"  # 算法名称改为ELP
    exp_remark = "修改版ELP+getfitness1+种群优化作为其中一个动作"
    exp_number = 1
    is_exp = True
    ...
    k = 0.1  # 系数k（可根据实验调整）
    Q_matrix = np.zeros((1, 6))  # Q值矩阵（1个状态，5个操作符，初始为0）
